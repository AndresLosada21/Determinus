// eslint-disable-next-line @typescript-eslint/ban-ts-comment -- The v1-to-v2 compatibility adapter is incrementally typed.
// @ts-nocheck
/**
 ...
 */
function buildFactoryFailureHooks(error: Error, directory: string): any {
...
export default Plugin.define({
  ...
  setup: async (ctx: any) => {
    ...
    const worktree: string = directory;
    const project: any = ctx.location.project
      ? {
          id: (ctx.location.project as any).id ?? directory,
          worktree: directory,
          vcsDir: (ctx.location.project as any).directory,
          vcs: "git",
          time: { created: Date.now() },
        }
      : { id: directory, worktree: directory, time: { created: Date.now() } };

    // Shim client for old code that expects @opencode-ai/plugin client
    const shimClient: any = {
      session: {
        get: async ({ path: { id } }: { path: { id: string } }) => {
          try {
            const res: any = await ctx.session.get({ sessionID: id });
            const data = res?.data ?? res;
            // Ensure data has id and parentID
            return { data };
          } catch {
    ...
    // Register tools via v2 transform
    const toolMap: Record<string, any> = hooks.tool ?? {};
    if (Object.keys(toolMap).length > 0) {
      await ctx.tool.transform((draft: any) => {
        for (const [name, def] of Object.entries(toolMap)) {
          const rawArgs: Record<string, any> = (def as any).args ?? {};
          const hasArgs = Object.keys(rawArgs).length > 0;
          // Use zod schema for input; if no args, use empty object
          const inputSchema = hasArgs ? z.object(rawArgs) : z.object({});
          draft.add({
            name,
            ...
            // Preserve original execute with proper wrapping
            execute: async (input: unknown, toolCtx: any) => {
              const ctxForTool: any = {
                ...toolCtx,
                // Provide metadata/progress compatibility
                metadata: (meta: any) => {
                  if (typeof toolCtx.progress === "function")
                    return toolCtx.progress(meta);
                  if (typeof toolCtx.metadata === "function")
                    return toolCtx.metadata(meta);
                },
                ...
                directory,
                worktree,
                abort: toolCtx.abort ?? new AbortController().signal,
              };
              const result: any = await (def as any).execute(input, ctxForTool);
    ...
    if (hooks["tool.execute.before"]) {
      ...
      await ctx.tool.hook("execute.before", async (event: any) => {
        ...
        const output: any = { args: event.input };
        await beforeHook(input, output);
        // Propagate mutations
        if (output.args !== undefined) event.input = output.args;
        if ((input as any).tool !== event.tool)
          event.tool = (input as any).tool;
      });
    }
    if (hooks["tool.execute.after"]) {
      ...
      await ctx.tool.hook("execute.after", async (event: any) => {
        ...
        if (event.status === "completed") {
          ...
          if (typeof content === "string") outputStr = content;
          else if (Array.isArray(content)) {
            const texts = (content as any[])
              .filter((c) => c?.type === "text" && typeof c.text === "string")
              .map((c) => c.text)
              .join("\n");
            outputStr = texts || JSON.stringify(content);
          } else if (event.result?.output !== undefined) {
            outputStr =
        ...
        if (event.status === "completed" && output) {
          if (
            ...
            event.result = { ...event.result, metadata: output.metadata };
          } else if (output.result && output.result !== event.result) {
      ...
      (async () => {
        try {
          for await (const ev of ctx.event.subscribe({
            ...
            // Old handler signature: async ({ event }) => void where event.type and event.properties
            try {
              await eventHook({ event: { type, properties, ...evAny } } as any);
            } catch (e) {
              debugLog(`event hook failed for ${type}: ${e}`);
            }
          }
        } catch (e: any) {
          if (e?.name !== "AbortError")
            debugLog(`event subscription loop ended: ${e}`);
        }
      })();
    }
    ...
    return async () => {
      ...
      try {
        // Old plugin's shutdown handlers were registered globally; we can try to call cleanup if available
        // The old impl's registerShutdownHandlers returned removeProcessListeners which we don't have reference to
        // But we can call cleanupTerminal to clean status UI
        cleanupTerminal();
      } catch {
