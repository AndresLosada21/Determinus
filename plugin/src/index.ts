 ...
 * Unlike `compactLargeText` (head/tail excerpt â€” retained for DIFF patches,
...
const advancePluginImpl: Plugin = async (input) => {
  ...
  // P2.7: legacy migration removed â€” disk-only store reads/writes the same
  ...
  // AC6 â€” reset session-scoped metrics on every plugin init. JC-1 keeps
  // metrics in-memory only; no persistence across plugin restarts.
  resetMetrics();
  resetCacheTokenTelemetry();
  resetLaneProjectionsCache();
  // AC1 â€” schema conversion is amortized at init; request hooks only read the
  ...
  // emit informational marker only â€” per-worktree git isolation eliminates
  // working-tree races).
  ...
  const handleToolExecuteBefore = async (
    ...
    if (shouldRepointActiveChange(toolName, args)) {
      if (args.changeId === state.activeChange.id) {
        ...
        // repeat calls â€” if the caller's pointer already matches, no work).
        ...
        try {
          ...
          // Cross-project uses disk-only check (KD3) â€” Visibility and
        ...
        // Same-project reachability gate (AC4/AC5) â€” check before re-pointing
        try {
   ...
   *
   * The pane identity contract renders stable IDs only â€” display titles
   * never enter the title path. Returns `epicId` when the change has
   * Epic membership; otherwise an empty object. Falls back gracefully â€”
  ...
  return {
    ...
    "experimental.session.compacting": async (
      ...
      try {
        ...
        // active state â€” the snapshot still produces useful output.
        ...
        // effort â€” a derivation failure must not break compaction output.
 ...
 * in its function schema and zero diagnostic surface â€” the original
...
export const AdvancePlugin: Plugin = async (input) => {
  try {
    ...
    debugLog(
      `AdvancePlugin factory threw: ${error.message} â€” registering degraded hooks`,
    );
    return buildFactoryFailureHooks(error, input.directory);
  }
};

// V2 compatibility: opencode v2 expects Plugin.define({id, setup}) or object with id+setup.
// We keep v1 function export for backward compat and also expose a hybrid object
// that supports both `await plugin(input)` (v1) and `await plugin.setup(ctx)` (v2).
// Host detection: v2 calls setup with Context {location, tool, session...}, v1 calls as function.
const AdvancePluginV2: any = {
  id: "determinus",
  setup: async (ctx: any) => {
    // Derive directory from v2 Context or fallback
    const directory: string =
      ctx?.location?.directory ||
      ctx?.directory ||
      ctx?.worktree ||
      process.cwd();
    const worktree: string = ctx?.location?.worktree || directory;
    const project: any = ctx?.project || { worktreeID: directory, directory };
    const client: any = ctx?.client || ctx;
    const serverUrl: URL =
      ctx?.serverUrl || new URL("http://localhost:9876");
    // Build v1-style input and delegate to existing impl
    const input: any = {
      directory,
      worktree,
      project,
      client,
      serverUrl,
      experimental_workspace: ctx?.experimental_workspace || {
        register: () => {},
      },
      $: ctx?.$,
    };
    let hooks: any;
    try {
      hooks = await advancePluginImpl(input);
    } catch (e) {
      const error = e instanceof Error ? e : new Error(String(e));
      debugLog(`AdvancePluginV2 setup: impl threw ${error.message}`);
      hooks = buildFactoryFailureHooks(error, directory);
    }

    // Register tools via v2 ctx.tool.transform if available
    if (ctx?.tool?.transform && hooks?.tool) {
      try {
        await ctx.tool.transform((draft: any) => {
          for (const [name, def] of Object.entries<any>(hooks.tool as any)) {
            try {
              // v1 def is {description, args, execute} via tool() helper
              // v2 draft.add expects {name, description, input/output, execute}
              // We bridge Zod args as-is via `as any` (host tolerates both)
              const inputSchema = (def as any).args;
              const execute = (def as any).execute;
              draft.add({
                name,
                description: (def as any).description,
                // host v2 uses `input` (Effect Schema) but we pass Zod via any
                input: inputSchema,
                // Provide both `input` and `args` for compat
                args: inputSchema,
                execute,
              } as any);
            } catch (e) {
              debugLog(`tool bridge add failed for ${name}: ${e}`);
            }
          }
        });
      } catch (e) {
        debugLog(`ctx.tool.transform failed: ${e}`);
      }
    } else if (ctx?.tool?.add && hooks?.tool) {
      // Fallback direct adds
      for (const [name, def] of Object.entries<any>(hooks.tool as any)) {
        try {
          await ctx.tool.add({
            name,
            description: (def as any).description,
            input: (def as any).args,
            execute: (def as any).execute,
          } as any);
        } catch {}
      }
    }

    // Bridge other hooks where domains exist
    // event, tool.execute.before/after, experimental* are handled inside advancePluginImpl via closure state,
    // but for v2 we also need to wire them if the host provides hook registration points.
    // We attempt best-effort wiring for the most critical hooks:
    try {
      if (ctx?.tool?.hook && hooks?.["tool.execute.before"]) {
        await ctx.tool.hook("execute.before", hooks["tool.execute.before"] as any);
      }
      if (ctx?.tool?.hook && hooks?.["tool.execute.after"]) {
        await ctx.tool.hook("execute.after", hooks["tool.execute.after"] as any);
      }
      if (ctx?.event?.hook && hooks?.event) {
        await ctx.event.hook("event", hooks.event as any);
      }
      // Experimental hooks: try session hooks if available
      if (ctx?.session?.hook) {
        if (hooks?.["experimental.chat.messages.transform"]) {
          await ctx.session.hook("messages.transform", hooks["experimental.chat.messages.transform"] as any).catch(()=>{});
        }
        if (hooks?.["experimental.session.compacting"]) {
          await ctx.session.hook("compacting", hooks["experimental.session.compacting"] as any).catch(()=>{});
        }
      }
    } catch (e) {
      debugLog(`hook bridge failed: ${e}`);
    }

    // Return cleanup that delegates to original's cleanup if any
    return async () => {
      try {
        if (typeof hooks?.cleanup === "function") await hooks.cleanup();
        cleanupTerminal();
      } catch {}
    };
  },
...
// V2 default: object with id+setup (opencode2 strict schema expects object, not function)
// Keep AdvancePlugin named export for v1 compat, but default must be object for opencode2
export default AdvancePluginV2 as any;
