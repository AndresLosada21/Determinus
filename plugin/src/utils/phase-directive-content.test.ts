 ...
 *
 * Pins the authored /determinus-review happy-path procedure delivered via
...
import {
  PHASE_DIRECTIVES,
  determinus_REVIEW_DIRECTIVE_CONTENT,
...
const REQUIRED_TOKENS: string[] = [
  ...
  'skill("determinus-frontend-review")',
  "determinus-designer",
  "apply-phase only",
  "MUST NOT be spawned",
...
describe("phase-directive-content registry", () => {
  test("registry exposes the determinus-review directive, schema-valid at load", () => {
    const entry = PHASE_DIRECTIVES["determinus-review"];
    expect(entry).toBeDefined();
    expect(entry.kind).toBe("phase_directive");
    expect(entry.command).toBe("determinus-review");
  ...
  test("contentHash is bound to content by construction (DD3)", () => {
    ...
    expect(entry.contentHash).toBe(
      sha256Hex(determinus_REVIEW_DIRECTIVE_CONTENT),
    );
  });
  ...
  test("template-literal escape integrity: no unresolved escapes, no interpolation junk", () => {
    ...
    expect(determinus_REVIEW_DIRECTIVE_CONTENT).not.toContain("\\`");
    expect(determinus_REVIEW_DIRECTIVE_CONTENT).not.toContain("\\${");
    expect(determinus_REVIEW_DIRECTIVE_CONTENT).not.toContain("${");
    // Placeholders survive verbatim.
    expect(determinus_REVIEW_DIRECTIVE_CONTENT).toContain("{change-id}");
    expect(determinus_REVIEW_DIRECTIVE_CONTENT).toContain("{workdir}");
  });

  test("code-fence parity: fenced blocks are balanced", () => {
    const fences = determinus_REVIEW_DIRECTIVE_CONTENT.match(/```/g) ?? [];
  ...
  test("12-dimension framework is single-source inside the directive", () => {
    expect(
      determinus_REVIEW_DIRECTIVE_CONTENT.match(/^\| 1 \| Design \|/gm) ?? [],
    ).toHaveLength(1);
    expect(
      determinus_REVIEW_DIRECTIVE_CONTENT.match(/^\| 12 \| Consistency \|/gm) ??
        [],
    ).toHaveLength(1);
    const methodologyIdx = determinus_REVIEW_DIRECTIVE_CONTENT.indexOf(
      "### Review Methodology",
    );
    const frameworkIdx = determinus_REVIEW_DIRECTIVE_CONTENT.indexOf(
      "## 12-Dimension Review Framework",
    );
  ...
  test("checkpoint ordering: preflight < consequence context < persist < ask < gate completion", () => {
    const c = determinus_REVIEW_DIRECTIVE_CONTENT;
  ...
  test("required contract tokens are present verbatim (C5)", () => {
    const missing = REQUIRED_TOKENS.filter(
      (token) => !determinus_REVIEW_DIRECTIVE_CONTENT.includes(token),
    );
  ...
  test("forbidden tokens are absent", () => {
    const present = FORBIDDEN_TOKENS.filter((token) =>
      determinus_REVIEW_DIRECTIVE_CONTENT.includes(token),
    );
  ...
  test("Target Resolution and frontmatter are NOT in the directive (launcher-owned)", () => {
    ...
    expect(determinus_REVIEW_DIRECTIVE_CONTENT).not.toContain("phaseGoal:");
  });
});
