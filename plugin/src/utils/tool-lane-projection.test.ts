...
describe("tool-lane-projection", () => {
  test("returns available projection when opencode debug agent resolves permissions", async () => {
    ...
    mockExecFile.mockImplementation(
      (
        ...
        const lane = args[2];
        if (lane === "determinus-ci-waiter") {
          callback(
            null,
            '{"level":"info"}\n{"tools":{"determinus_status":true,"determinus_engineer":false}}',
            "",
          );
        } else {
          callback(
            null,
            '{"tools":{"determinus_status":true,"determinus_engineer":true}}',
            "",
          );
        }
      },
    ...
    expect(projections["determinus-ci-waiter"]).toEqual({
    ...
    expect(projections["determinus-engineer"]).toEqual({
      availability: "available",
      enabled_tools: 2,
      schema_bytes: 200,
  ...
  test("returns unavailable projection when opencode debug agent fails", async () => {
    ...
    const projection = await getLaneProjection(manifest, "determinus-engineer");
