...
describe("formatAdvToolTitle", () => {
  test("formats representative ADV tool titles", () => {
    ...
    expect(
      formatAdvToolTitle("determinus_task_update", { taskId: "tk-123" }).title,
    ).toBe("Update task: tk-123");
    expect(
      formatAdvToolTitle("determinus_gate_complete", { gateId: "planning" })
        .title,
    ).toBe("Complete gate: planning");
    expect(
      formatAdvToolTitle("determinus_run_test", { command: "pnpm test" }).title,
    ).toBe("Run test: pnpm test");
    expect(
      formatAdvToolTitle("determinus_ops_run_upsert", {
        runId: "run-1",
      }).title,
  ...
  test("redacts sensitive values from title strings", () => {
    const result = formatAdvToolTitle("determinus_run_test", {
  ...
  test("strips terminal control sequences from title strings", () => {
    const result = formatAdvToolTitle("determinus_change_create", {
      summary:
  ...
  test("distinguishes explicit coverage from fallback titles", () => {
    expect(hasExplicitAdvToolTitle("determinus_change_show")).toBe(true);
    expect(hasExplicitAdvToolTitle("determinus_future_tool")).toBe(false);
    expect(formatAdvToolTitle("determinus_future_tool", {}).title).toBe(
      "Future tool",
    );
  });
  ...
  test("uses stable generic titles for weak-key tools", () => {
    expect(formatAdvToolTitle("determinus_change_list", {}).title).toBe(
      "List changes",
    );
    expect(formatAdvToolTitle("determinus_task_ready", {}).title).toBe(
      "Show ready tasks",
    );
    expect(formatAdvToolTitle("determinus_wip_state", {}).title).toBe(
      "Show WIP state",
    );
  });

  test("includes display-only namespaced metadata", () => {
    const result = formatAdvToolTitle("determinus_gate_status", {
      changeId: "fixThing",
    });

    expect(result.metadata).toEqual({
      adv: {
        toolName: "determinus_gate_status",
