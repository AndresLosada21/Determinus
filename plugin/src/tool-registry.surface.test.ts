...
describe("getToolSurface (live surface)", () => {
  ...
  test("exposes a known real tool arg (determinus_change_create#target_path)", () => {
    expect(surface.get("determinus_change_create")?.has("target_path")).toBe(
      true,
    );
  });

  test("does not expose removed public recovery args on routine mutation tools", () => {
    const args = surface.get("determinus_change_close");
  ...
  test("exposes determinus_change_archive#target_path for cross-project archive", () => {
    expect(surface.has("determinus_change_archive")).toBe(true);
    expect(surface.get("determinus_change_archive")?.has("target_path")).toBe(
      true,
    );
  });

  test("exposes determinus_task_checkpoint#target_path for cross-project checkpoint", () => {
    expect(surface.has("determinus_task_checkpoint")).toBe(true);
    expect(surface.get("determinus_task_checkpoint")?.has("target_path")).toBe(
      true,
    );
  });
...
- AC1: Cross-project work routes through target. [warrant: tool:determinus_change_create#target_path]
`,
    ...
    expect(contract.items[0]?.warrants).toEqual([
      "tool:determinus_change_create#target_path",
    ]);
  });
