...
function parseTree(text, errors = [], options = ParseOptions.DEFAULT) {
  ...
  const visitor = {
    ...
    onLiteralValue: (value2, offset, length) => {
      onValue({ type: getNodeType(value2), offset, length, parent: currentParent, value: value2 });
      ensurePropertyComplete(offset + length);
    },
...
function setProperty(text, originalPath, value2, options) {
  ...
  while (path.length > 0) {
    ...
    if (parent === void 0 && value2 !== void 0) {
      if (typeof lastSegment === "string") {
        value2 = { [lastSegment]: value2 };
      } else {
        value2 = [value2];
      }
    } else {
  ...
  if (!parent) {
    ...
    return withFormatting(text, { offset: root2 ? root2.offset : 0, length: root2 ? root2.length : 0, content: JSON.stringify(value2) }, options);
    ...
    if (existing !== void 0) {
      if (value2 === void 0) {
        ...
        return withFormatting(text, { offset: existing.offset, length: existing.length, content: JSON.stringify(value2) }, options);
      }
    } else {
      if (value2 === void 0) {
        return [];
      }
      const newProperty = `${JSON.stringify(lastSegment)}: ${JSON.stringify(value2)}`;
    ...
    if (insertIndex === -1) {
      const newProperty = `${JSON.stringify(value2)}`;
      let edit;
      ...
      return withFormatting(text, edit, options);
    } else if (value2 === void 0 && parent.children.length >= 0) {
      ...
      throw new Error(`Can not ${value2 === void 0 ? "remove" : options.isArrayInsertion ? "insert" : "modify"} Array index ${insertIndex} as length is not sufficient`);
    }
  } else {
...
function modify(text, path, value2, options) {
  return setProperty(text, path, value2, options);
}
...
var normalize = (x) => x.replace(/\\/g, "/").replace(/^file:\/\//, "").replace(/\/$/, "").toLowerCase();
function managedPlugin(value2) {
  const item = typeof value2 === "string" ? value2 : value2 && typeof value2 === "object" && "package" in value2 ? value2.package : "";
  if (typeof item !== "string") return false;
  const name = normalize(item);
  return /^(?:@andreslosada21\/)?(?:determinus|advance)(?:@[^/]+)?$/.test(name) || /(?:^|\/)(?:determinus(?:-[^/]*)?|advance(?:-[^/]*)?|advance-opencode2)(?:\/|$)/.test(
    name
  );
}
function patchConfig(text, entry) {
  const errors = [];
  const config = parse2(text.replace(/^\uFEFF/, ""), errors, { allowTrailingComma: true }) ?? {};
  if (errors.length || !config || typeof config !== "object" || Array.isArray(config))
    throw Error(
      "Invalid OpenCode JSONC: " + errors.map((e) => printParseErrorCode(e.error)).join(", ")
    );
  let result = text.replace(/^\uFEFF/, "");
  const opts = {
    formattingOptions: { insertSpaces: true, tabSize: 2, eol: "\n" }
  };
  if (config.plugin !== void 0) {
    const items = Array.isArray(config.plugin) ? config.plugin : [config.plugin];
    const remaining = items.filter((x) => !managedPlugin(x));
    result = applyEdits(
      result,
      modify(
        result,
        ["plugin"],
        remaining.length ? remaining : void 0,
        opts
      )
    );
  }
  if (config.plugins !== void 0 && !Array.isArray(config.plugins))
    throw Error("OpenCode plugins must be an array");
  const entries = (config.plugins ?? []).filter(
    (x) => !managedPlugin(x) && !(typeof x === "string" && normalize(x) === normalize(entry))
  );
  result = applyEdits(
    result,
  ...
  if (config.instructions !== void 0) {
    ...
    const remaining = items.filter(
      (x) => typeof x !== "string" || !/(?:^|[/\\])ADV_INSTRUCTIONS\.md$/i.test(x)
    );
...
function verifyRelease(root2) {
  const manifest = JSON.parse(
    readFileSync(join(root2, "release-manifest.json"), "utf8")
  );
  if (manifest.version !== "3.6.0-beta.1" || !manifest.files || typeof manifest.files !== "object")
  ...
  const bundle = JSON.parse(
    readFileSync(join(root2, "plugin/dist/plugin-bundle-manifest.json"), "utf8")
  );
  for (const [name, hash] of Object.entries(bundle.files)) {
    if (!/^[a-z-]+$/.test(name) || sha(readFileSync(join(root2, "plugin/dist", name + ".js"))) !== hash)
      throw Error("Bundle checksum mismatch");
  }
  return {
    manifest,
    bundle,
    identity: sha(readFileSync(join(root2, "release-manifest.json"))).slice(
      0,
      16
    )
  };
}
var encode = (file) => existsSync(file) ? readFileSync(file).toString("base64") : null;
function atomic(file, body) {
  mkdirSync(dirname(file), { recursive: true, mode: 448 });
  const temp = file + ".tmp-" + randomBytes(6).toString("hex");
  writeFileSync(temp, body, { mode: 384 });
  renameSync(temp, file);
}
function rollback(j, path) {
...
function installRelease(root2, home, options = {}) {
  ...
  const configDirs = [
    .../* @__PURE__ */ new Set([
      globalConfig,
      ...options.project ? [resolve(options.project), ...discoveryDirs.slice(1)] : []
    ])
  ];
  const configs = [
    .../* @__PURE__ */ new Set([
      ...configDirs.flatMap((dir, i) => {
        ...
        return found.length ? found : i === 0 ? [join(dir, "opencode.jsonc")] : [];
      }),
      ...options.configFile ? [resolve(options.configFile)] : []
    ])
  ];
  const writes = configs.map((file) => ({
    file,
    before: encode(file),
    after: Buffer.from(
      patchConfig(
  ...
  writes.push({
    file: agent,
    before: encode(agent),
    after: readFileSync(join(root2, ".opencode/agents/determinus.md")).toString(
      "base64"
    )
  });
  if (options.dryRun)
    return { status: "plan", target, configs, version: manifest.version };
  mkdirSync(deploy, { recursive: true, mode: 448 });
  ...
  if (existsSync(lock)) {
    ...
    rmSync(lock);
    return installRelease(root2, home, options);
  }
  const backup = join(
    deploy,
    "backups",
    (/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-") + "-" + randomBytes(4).toString("hex")
  ...
  const journal = { status: "pending", writes, moves: [] }, stage = target + ".stage-" + process.pid;
  try {
    if (!existsSync(target)) {
      ...
      for (const name of Object.keys(manifest.files)) {
        ...
        writeFileSync(dest, readFileSync(join(root2, name)));
      }
      writeFileSync(
        join(stage, "release-manifest.json"),
        readFileSync(join(root2, "release-manifest.json"))
      );
      verifyRelease(stage);
    ...
    for (const dir of new Set(discoveryDirs)) {
      ...
      for (const sub of ["agents", "agent", "skills", "command", "commands"]) {
        ...
        for (const name of readdirSync(folder)) {
          if (!/^(?:adv|advance)(?:\.md)?$/i.test(name) && !/^determinus-.+/i.test(name) && !(sub === "skills" && name === "determinus"))
            continue;
...
function rollbackInstalled(home) {
  const deploy = join(home, ".local/share/Determinus"), lock = join(deploy, "install.lock");
  const receipt = JSON.parse(
    readFileSync(join(deploy, "installed.json"), "utf8")
  ), path = join(receipt.backup, "transaction.json"), journal = JSON.parse(readFileSync(path, "utf8"));
...
// scripts/install.ts
var root = dirname2(fileURLToPath(import.meta.url));
...
try {
  ...
  const userRoot = resolve2(value("--home") ?? homedir());
  if (args.includes("--rollback")) {
    ...
    if (process.env.OPENCODE_CONFIG_CONTENT) {
      const errors = [];
      const inline = parse2(process.env.OPENCODE_CONFIG_CONTENT, errors, {
    ...
    const probe = spawnSync(
      ...
      [
        "--input-type=module",
        "-e",
        `const x=await import(${JSON.stringify(pathToFileURL(resolve2(root, "plugin/dist/index.js")).href)});if(x.default?.id!=='determinus'||typeof x.default.setup!=='function')process.exit(2);`
      ],
      { encoding: "utf8", timeout: 3e4 }
    );
    if (probe.status !== 0)
      throw Error(
        "Compiled bundle could not be imported: " + (probe.stderr || probe.error?.message)
      );
