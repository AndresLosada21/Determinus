---
name: determinus-frontend-review
...
metadata:
  priority: medium
  source: determinus-designer-followup
---
...
Canonical methodology for reviewing UI/component work delivered by `determinus-designer` (or any frontend implementation, including engineer-implemented UI). Replaces the inline 6-dimension checklist that previously lived in `.opencode/command/determinus-review.md` and `.opencode/command/determinus-harden.md` Reviewer Remediation Packets.
...
The 6 dimensions are set law — they originate in the `addAdvDesigner` agreement (AC3, DESIGN QUALITY BAR) and are mirrored verbatim by `determinus-designer`'s prompt. Don't add, drop, or rename them inside this skill without a coordinated change across `determinus-designer.md`, `determinus-review.md`, `determinus-harden.md`, and `plugin/src/determinus-designer-assets.test.ts`.

## When to Load

Load explicitly via `skill("determinus-frontend-review")` only when:

1. You are `determinus-reviewer` and the Reviewer Remediation Packet `FRONTEND DESIGN REVIEW SKILL` anchor instructs you to.
...
× Do not load this skill as `determinus-designer` (apply-phase only) or as any other role. Review/harden ownership stays with `determinus-reviewer`.

## The 6 Dimensions

Each dimension MUST be assessed when reviewing a change that touches frontend/component code. Use the conventional comment labels from `/determinus-review` Phase 0 (`blocker:`, `issue:`, `suggestion:`, `nit:`, `question:`, `praise:`) when emitting findings.
...
- `determinus-reviewer` owns review and harden gates. This skill provides methodology only — it does NOT reassign ownership.
- `determinus-designer` is apply-phase only (per `addAdvDesigner` agreement). It MUST NOT be spawned for review or harden remediation. If you find yourself wanting to delegate a fix to `determinus-designer` during review, you are operating outside the contract — either fix inline (as `determinus-reviewer` with scoped repo-write capability) or surface the fix as a follow-up task that re-enters the apply gate.
- For backend findings in a mixed change, route remediation through `determinus-engineer`, not `determinus-designer` — backend ownership belongs to engineer regardless of the change's frontend scope.

## Anti-Patterns

- **Auto-loading without packet instruction.** This skill should only load when the reviewer packet explicitly references `skill("determinus-frontend-review")`. Loading speculatively wastes context.
...
1. `skills/determinus-frontend-review/SKILL.md` (this file)
2. `.opencode/command/determinus-review.md` Reviewer Remediation Packet inline fallback
3. `.opencode/command/determinus-harden.md` Reviewer Remediation Packet inline fallback
4. `.opencode/agents/determinus-designer.md` DESIGN QUALITY BAR section
5. `plugin/src/determinus-designer-assets.test.ts` dimension-enumeration test
6. `plugin/src/determinus-reviewer-asset.test.ts` packet anchor assertion
