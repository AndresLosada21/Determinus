...
metadata:
  priority: medium
  source: determinus-cleanup-command
---
...
Methodology for `/determinus-cleanup`: triage all four ADV hygiene surfaces, bucket candidates by reversibility, report them, and guide command-owned application after strict approval. Skill is read-only guidance; command owns ADV tool calls, state mutation, and approval enforcement.
...
- No sub-agents.
- Never auto-archive; recommend `/determinus-archive {id}`.
- Never purge state leaks; recommend `determinus_archive_purge`, which owns its own operator approval.
...
> **DESTRUCTIVE DEFAULT WARNING:** During Phase 1 discovery, omitting `dryRun: true` from `determinus_worktree_cleanup` **DELETES branches instead of listing them**; the tool has no safe default. `dryRun: true` is mandatory for discovery. Apply deletion only in Phase 5 after typed confirmation.
...
Surface 4 stays report-only: leak remediation routes to `determinus_archive_purge`, an operator-only recovery tool with its own approval contract.
...
```text
## /determinus-cleanup triage report
...
- {id} ({tasks done}/{total} tasks, all gates except release done)
  → Run `/determinus-archive {id}` to ship.
...
- {id} — {leak class}
  → Run `determinus_archive_purge` for {id} (operator-only; owns its own approval).
...
- Worktree deletion → `determinus_worktree_delete` per approved branch.
- Branch deletion → `determinus_worktree_cleanup mode: "archived_branches" changeId: <approved candidate change ID>` without `dryRun`, once per approved candidate. Never issue an unscoped merged-branch cleanup apply: it would delete every currently eligible branch and bypass a subset approval.
...
| × Bad | ✓ Good |
|---|---|
| Bulk-archive ready bucket | Recommend per-change `/determinus-archive {id}` |
...
| Treat `determinus_worktree_triage` safe group as deletion authority | Delegate to `determinus_worktree_delete` and let it re-verify |
...
| Call determinus_worktree_cleanup without dryRun during discovery | `dryRun: true` is mandatory in Phase 1; the tool has no safe default |
| Omit an empty surface from the report | Render an explicit empty state |
| Purge state leaks from cleanup | Recommend `determinus_archive_purge`, which owns its own approval |
...
- Deletion authority belongs to `determinus_worktree_delete` / `determinus_worktree_cleanup` alone.
