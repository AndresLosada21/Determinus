...
- `cancel` / `stop` — halt the entire /determinus-triage run
...
- `stop` / `abort` — halt the entire /determinus-triage run
...
After approval, call `determinus_change_update_issues` once per approved pair. Hidden overflow requires a separate prompt. Partial failures preserve successful links and report exact retry calls.
