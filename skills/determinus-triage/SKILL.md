...
Methodology for `/determinus-triage`: reconcile source items into GitHub issues, apply bug priorities autonomously, coalesce approved issue↔change overlaps, and report what to finish, clean up, or start across active changes, Epics, and open issues.
...
**Canonical source:** `.opencode/command/determinus-triage.md` owns orchestration and mutation. This skill owns rubrics, prompt templates, schemas, and anti-patterns.
...
7. **Coalesce** — exclude existing links, classify remaining issue↔change pairs as structural or heuristic, show bounded evidence, and link only displayed user-approved pairs with `determinus_change_update_issues`.
...
- Coalesce scope is active changes↔open issues. Change↔change duplicate handling stays with `/determinus-cleanup`.
