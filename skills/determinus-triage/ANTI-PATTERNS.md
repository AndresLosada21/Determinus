# determinus-triage Anti-Patterns + Coexistence

## Coexistence

| Command | Role | Relationship to `/determinus-triage` |
|---|---|---|
| `/determinus-status` | Read-only project overview | Health/status counterpart, not portfolio prioritization |
| `/determinus-cleanup` | Close/archive abandoned, duplicate, or completed ADV changes | Owns cleanup mutations surfaced by triage |
| `/determinus-epic` | Create/update Epic initiative containers | Supplies initiative context; triage never mutates membership implicitly |
| `/determinus-proposal` | Start issue-linked work | Consumes Open issues worth solving pointers |
| `/determinus-idea` / `/determinus-problem` | Shape ideas or bugs | Their durable outputs may enter triage inventory |
...
| Detect change↔change duplicates in Coalesce | Delegate duplicate/superseded changes to `/determinus-cleanup` |
| Close/cancel/archive changes from Portfolio balance | Report bucket counts/IDs and point to `/determinus-cleanup` |
...
| Recommend removed `/determinus-roadmap`, `adv roadmap`, or the retired portfolio reader MCP tool | Route portfolio requests to `/determinus-triage` |
...
6. Call `determinus_change_update_issues` for approved displayed pairs.
