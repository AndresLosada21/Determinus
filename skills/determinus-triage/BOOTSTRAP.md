...
1. Read `determinus_project_metadata key: 'github_project'`.
...
Project may retain legacy scoring fields (`Value`, `TimeCriticality`, `RROE`, `Effort`, `WSJF`) for backward compatibility, but `/determinus-triage` treats them as read-only. Do not create new scoring fields.
...
| Active ADV changes | `determinus_change_list status: 'in-flight'` | id, title, summary |
| Active ADV Epics | MCP Tier-4 `tools.adv.epic_list` then bounded `tools.adv.epic_show` for plausible relevant Epics | Bridged by `plugin/src/mcp-server/tier4-tool-map.ts`; reachable under Code Mode, not host `determinus_*` tools; extract id, title, narrative, entry order/status |
| ADV wisdom | `determinus_wisdom_list type: 'failure'` then `type: 'gotcha'` | snippets |
