...
metadata:
  priority: medium
  source: determinus-reflect-command
---
...
- `determinus_reflect` persists structured two-plane report to `reflections.jsonl`.
...
- Reflection failure from `/determinus-archive` is warning-only and MUST NOT block archive completion.
...
- No archive blocking when invoked from `/determinus-archive`.
