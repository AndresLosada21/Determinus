...
| ADV worktrees | Use `determinus_worktree_resume` / `determinus_worktree_create`; tool owns paths. |
| PR source | Worktree, never trunk; CI behavior matches trunk pushes. |
| Locations | ADV: `$determinus_WORKTREE_HOME/{projectId}/{branch}`; ad-hoc: `~/.local/share/opencode/worktree-adhoc/{repo-basename}/{branch-slug}`. |
