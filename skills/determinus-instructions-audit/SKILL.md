---
name: determinus-instructions-audit
...
metadata:
  priority: medium
  source: determinus-instructions-audit-skill
---

# ADV Instructions Audit

## Purpose
Read-only methodology: treat Advance's `*-assets.test.ts` / invariant / drift test files as the **anchor layer** for instruction prose, and audit instruction surfaces against it. Adds no enforcement; makes the ad-hoc anchor layer systematic and discoverable. Different axis from `determinus-audit` (spec↔code) — this is instruction-prose↔anchor.

## When to use
- After editing any instruction surface (`AGENTS.md`, `project.md`, `determinus_INSTRUCTIONS.md`, `.opencode/command|agents/`, skills).
...
3. **Single-source-per-surface** — each surface owns one claim kind (`project.md`=agent context, `AGENTS.md`=dev ramp-up, `determinus_INSTRUCTIONS.md`=full protocol, `command/*.md`=per-gate contract, `specs/`=laws); others point, never restate.
...
| Kind | Enumerate via |
|---|---|
| Surface-asset | `rg -l 'AGENTS\.md\|determinus_INSTRUCTIONS\.md\|project\.md' plugin/src/**/*.test.ts` |
| Invariant | filename `*invariant*` / `*no-psw*` |
| Boundary | `workflow-bundle-boundary`, `context-snapshot.purity` |
| Per-command | filename `determinus-<cmd>-assets.test.ts` |
...
## Dry-run procedure
1. Pick ≥3 surfaces (e.g. `project.md`, `AGENTS.md`, one `determinus_INSTRUCTIONS.md` section).
...
## Constraints
- Read-only; never auto-fix (mirrors `determinus-audit`).
- Methodology-only SKILL.md; no standing inventories (`rq-skillProseCompression01` / `advance-meta.md`).
- Different axis from `determinus-audit`; `determinus-instructions-assets.test.ts` and the runtime-coverage doc are anchor *instances*/prior art, not competitors.
