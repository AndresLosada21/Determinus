...
metadata:
  priority: medium
  source: determinus-clarify-command
---
...
Triggered from `/determinus-discover` Phase 2.5 when ambiguity threshold hits CRITICAL ≥ 1 or HIGH ≥ 2. Also accepts findings from `/determinus-audit` Phase 3 inline ambiguity scan.

Input findings follow `determinus_INSTRUCTIONS.md § Ambiguity Taxonomy`:
...
6. Emit REQUIREMENTS DISCOVERY SUMMARY and return instruction: `Next: rerun /determinus-discover {change-id}` or `Next: rerun /determinus-audit {capability}`.
...
If invoked from `/determinus-discover`, final line: `Next: rerun /determinus-discover {change-id}`.
