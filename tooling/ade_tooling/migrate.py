...
def migrate(*, target: Path | None = None, force: bool = False, skip_runtime_check: bool = False) -> dict:
    ...
    if not (current.startswith("4.") or current.startswith("5.0.") or current.startswith("5.1.") or current in {"5.2.0","5.2.1","5.2.2","5.2.3","5.2.4","5.2.5","5.2.6"}):
        raise ADEError(f"MIGRATION_BLOCKED: esperado v4.x/5.0.x/5.1.x/5.2.0/5.2.1/5.2.2/5.2.3/5.2.4/5.2.5/5.2.6; atual={current}")
