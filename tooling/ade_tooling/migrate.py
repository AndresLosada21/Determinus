...
def migrate(*, target: Path | None = None, force: bool = False, skip_runtime_check: bool = False) -> dict:
    ...
    if not (current.startswith("4.") or current.startswith("5.0.") or current.startswith("5.1.")):
        raise ADEError(f"MIGRATION_BLOCKED: esperado v4.x/5.0.x/5.1.x; atual={current}")
