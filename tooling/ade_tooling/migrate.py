...
PATCH_SOURCES = {"6.0.0", "6.0.1", "6.0.2", "6.0.3", "6.0.4"}
...
def migrate(*, target: Path | None = None, force: bool = False, skip_runtime_check: bool = False) -> dict:
    ...
    if current not in PATCH_SOURCES and not (current.startswith("4.") or current.startswith("5.")):
        raise ADEError(f"MIGRATION_BLOCKED: esperado ADE 6.0.0/6.0.1/6.0.2/6.0.3 ou v4.x/v5.x (major direto recomendado 5.2.8); atual={current}")
