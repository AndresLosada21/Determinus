...
Target: `/determinus-slop-scan` command and `determinus-slop-detection` skill
Mode: scoped
Created: 2026-06-11
Updated: 2026-06-11
...
This pack evaluates how `/determinus-slop-scan` can surface real tool-backed data for overengineering, dead code, bad abstractions, unneeded flexibility, and complexity-cost signals instead of relying primarily on agent prompting.

In scope:

- `.opencode/command/determinus-slop-scan.md`
- `skills/determinus-slop-detection/*`
- `slop-smells.yaml`
- `.adv/specs/slop-scan/spec.json` and `docs/specs/slop-scan.md`
- `plugin/src/determinus-slop-scan-assets.test.ts`
...
- Severity: MEDIUM
- Category: Security
- Evidence: `.opencode/command/determinus-slop-scan.md:57-63`, `skills/determinus-slop-detection/CATEGORIES.md:24-36`
...
- Follow-up: `/determinus-proposal Add slop detector runner`

### Reliability

- Severity: HIGH
- Category: Reliability
- Evidence: `.opencode/command/determinus-slop-scan.md:57-62`, `.opencode/command/determinus-slop-scan.md:143-150`, `bin/adv --help` output only lists `status` and `roadmap`; `bin/adv slop-scan --json` returns `unknown command: slop-scan`.
...
- Recommendation: Implement `adv slop-scan --json` as the canonical deterministic Phase 1 runner and let `/determinus-slop-scan` orchestrate around that output.
- Follow-up: `/determinus-proposal Add slop detector runner`
...
- Severity: HIGH
- Category: Testing
- Evidence: `plugin/src/slop-scan-false-positive-fixtures.test.ts:21-60`, `plugin/src/slop-scan-false-positive-fixtures.test.ts:90-153`, `plugin/src/determinus-slop-scan-assets.test.ts:21-215`
...
- Follow-up: `/determinus-task` or `/determinus-proposal Add slop detector runner`

### Observability

- Severity: MEDIUM
- Category: Observability
- Evidence: `.opencode/command/determinus-slop-scan.md:143-150`, `.opencode/command/determinus-slop-scan.md:151-158`, `docs/specs/slop-scan.md:528-564`
...
- Evidence: `docs/cli-surface-matrix.md:23-26` says `/determinus-slop-scan` is `mcp+cli-additive`; `bin/adv --help` output lists only `status` and `roadmap`; `bin/adv slop-scan --json` returns `unknown command: slop-scan`.
...
- Evidence: `.opencode/command/determinus-slop-scan.md:41`, `skills/determinus-slop-detection/CATEGORIES.md:5-12`, `plugin/src/types/project.ts:65-90`, `project.json:16-21`
...
2. **Structured JSON as source of truth** — Knip, ESLint, ast-grep, and scanner platforms expose JSON outputs. Relevance: `/determinus-slop-scan` can normalize tool JSON into one finding schema and reserve agents for interpretation only.
...
- Normalize existing tools into the current slop schema: `id`, `name`, `severity`, `file`, `line`, `description`, `fix`, `confidence`, `detectionMethod`, `grouping`, `actionability`, `phase`. Evidence: `.opencode/command/determinus-slop-scan.md:63`.
- Start with local TypeScript signals: ESLint complexity/depth, Knip dead-code/dependencies, ast-grep structural rules, regex signal layer from `skills/determinus-slop-detection/CATEGORIES.md:24-36`.
...
- Active changes shown by `determinus_change_list` do not directly cover `/determinus-slop-scan` runner work.
...
- `.opencode/command/determinus-slop-scan.md`
- `skills/determinus-slop-detection/SKILL.md`
- `skills/determinus-slop-detection/CATEGORIES.md`
- `skills/determinus-slop-detection/DEAD_CODE.md`
- `slop-smells.yaml`
