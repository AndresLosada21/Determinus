...
Target: broad repo scan focused on `determinus_*` tool latency, responsiveness, and throughput
Mode: broad
Created: 2026-05-24
Updated: 2026-05-24
...
- `plugin/src/tools/**` ADV MCP tool read paths, especially `determinus_status`, `determinus_change_list`, `determinus_change_show`, `determinus_task_list`, `determinus_roadmap`, and WIP/status helpers.
...
- Severity: HIGH
  ...
  Impact: `determinus_status view:"summary"` still builds the full status payload before projection. Summary calls pay for Temporal health, queue serviceability, search attributes, config load, recent-change enrichment, worktree cleanup discovery, worktree census, OpenCode session debt, health snapshot, snapshot-health scan, specs recount, peer sessions, plugin runtime provenance, and project metadata.
  Recommendation: Make `determinus_status` view-lazy: compute only fields needed by requested view. Keep `summary` to status + minimal health bit + worktree count; defer health/hygiene archaeology to their views.
  Follow-up: `/determinus-proposal Lazy determinus_status views`

- Severity: HIGH
  Category: Reliability
  Evidence: `plugin/src/tools/status.ts:1212-1227`
  Impact: `determinus_status` invokes `advWorktreeCleanup("status", ...)` on every status call. Even best-effort cleanup discovery adds I/O/DB work to a hot read tool and can amplify latency when worktree state is large or poisoned.
  Recommendation: Move cleanup retry/discovery out of default status, or run it only in `view:"hygiene"` / explicit `determinus_worktree_cleanup` with TTL caching.
  Follow-up: `/determinus-proposal Defer status cleanup work`
...
- Severity: MEDIUM
  ...
  Follow-up: `/determinus-proposal Align visibility project attributes`

### Testing

- Severity: MEDIUM
  Category: Testing
  Evidence: `plugin/package.json:31`, `plugin/scripts/bench-determinus-latency.ts:63-98`, `plugin/scripts/benchmark-execute.ts:4-17`, `plugin/src/temporal/list-change-workflows.test.ts:161-184`
  Impact: Bench tooling exists, but current automated guard evidence is narrow: synthetic `listChangeWorkflowIds` p99 only. No checked-in tool-level p50/p95 budget protects `determinus_status`, `determinus_change_list`, or `determinus_change_show` from regression.
  ...
  Follow-up: `/determinus-proposal Add ADV latency budgets`
...
- Severity: MEDIUM
  ...
  Impact: `determinus_PROFILE=1` records whole-tool durations, and metrics count tool calls, but in-process `wall_time_ms` is not wired into production hook output. Slow status substeps are not attributed in the health surface, so operators see “status is slow” rather than which probe/read caused it.
  Recommendation: Record per-tool wall time in `tool.execute.after`; add optional substep spans for `determinus_status` phases and expose hot tools/p95 in health view.
  Follow-up: `/determinus-proposal Add ADV latency telemetry`
...
- Severity: LOW
  ...
  Follow-up: `/determinus-task Document ADV latency workflow`
...
- Severity: HIGH
  ...
  Impact: List/status read paths still hydrate full changes. `listResolvedChanges` unions memo IDs, Visibility IDs, disk IDs, and archive IDs, then loads every change in batches. `determinus_status` then enriches every recent change with another `store.changes.get`, proposal read, context snapshot/ticker, dependency status, next gate, clarify checks, and recency recommendation.
  Recommendation: Introduce a true summary-index read model for `determinus_change_list`/`determinus_status` default views. Hydrate full change only for primary snapshot, `view:"changes"`, or explicit include flags.
  Follow-up: `/determinus-proposal Add change summary index`

- Severity: MEDIUM
  Category: Code Quality
  Evidence: `plugin/src/tools/change.ts:1875-1915`, `plugin/src/tools/change.ts:1951-2023`
  Impact: `determinus_change_show` always loads the proposal/problemStatement projections from `change.documents` and applies clarify readiness before optional include flags. Useful defaults, but phase-start flows that only need task/gate summaries still pay artifact I/O and checks.
  ...
  Follow-up: `/determinus-proposal Add lightweight change show`
...
- Lazy `determinus_status` view execution. Evidence shows all status work currently runs before projection (`plugin/src/tools/status.ts:847-850`, `plugin/src/tools/status.ts:1415-1416`). Expected user-visible gain: default orientation calls stop paying health/hygiene costs.
...
- `determinus_change_show` lightweight mode. Evidence shows artifact/clarify defaults run before include flags (`plugin/src/tools/change.ts:1875-1915`). Expected gain: phase-start and checkpoint flows can request just the state they need.
...
- Pending agenda `ag-aWL1Yd3T` “Reduce poisoned workflow WIP noise” overlaps WIP/status noise, but not the broader `determinus_status` lazy-view and summary-index work.
- Active/draft changes listed by `determinus_change_list` do not directly cover ADV tool latency as a primary objective.

## Open Questions for Research

1. What are current p50/p95/p99 latencies for `determinus_status`, `determinus_change_list`, `determinus_change_show`, `determinus_task_list`, `determinus_wip_state`, and `determinus_roadmap` on real project state with 10, 50, 250 changes?
2. Which `determinus_status` substep dominates on live project state: Temporal health, listResolvedChanges, recent-change enrichment, worktree cleanup, snapshot health, session debt, plugin runtime provenance, or project metadata?
...
4. Which current callers rely on default `determinus_change_show` artifact/clarify enrichment, and which can opt into lightweight shape without compatibility break?
...
- Local: `project.md` via `determinus_project_context`.
- Local: active changes via `determinus_change_list`, report follow-ups via `determinus_change_show`, specs via `determinus_spec action:"list"`.
- Local: `plugin/package.json:17-32`.
- Local: `plugin/scripts/bench-determinus-latency.ts:63-98`.
