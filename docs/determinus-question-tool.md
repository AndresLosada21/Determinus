...
See also: [docs/determinus-autonomy-compliance-matrix.md](determinus-autonomy-compliance-matrix.md) for the command-by-command autonomy audit.
...
**For structured pairwise or multi-candidate comparisons**, see [docs/user-intuit-protocol.md](user-intuit-protocol.md) and load `skill("determinus-user-intuit")` for a reusable presentation workflow (prepare candidates → format comparison → present via question tool → parse response).
