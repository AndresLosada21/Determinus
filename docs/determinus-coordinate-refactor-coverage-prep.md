...
Target: `/determinus-coordinate` command
Mode: scoped
Created: 2026-08-02
Updated: 2026-08-02
...
Assess whether `/determinus-coordinate` should surface and route refactoring work when
repository evidence shows a linked change or Epic is out of date.

In scope: command contract, Epic coordination law, existing `/determinus-refactor`
...
- Severity: HIGH
  ...
  Evidence: `.opencode/command/determinus-coordinate.md` Phase 3 compares linked
  ...
  `/determinus-refactor <change-id>`.

- Severity: MEDIUM
  Category: Reliability
  Evidence: `/determinus-refactor` only reconciles active change proposals and tasks
  (`.opencode/command/determinus-refactor.md`, Phases 1–4); its target resolution does
  not accept Epic IDs. `/determinus-coordinate` already owns approval-gated Epic
  narrative/reorder actions (Phases 7–9).
  Impact: treating an Epic as a `/determinus-refactor` target would promise a workflow
  that does not exist.
  ...
  action group; do not add an Epic mode to `/determinus-refactor` incidentally.

### Developer Experience

- Severity: MEDIUM
  Category: Developer Experience
  Evidence: `plugin/src/manifest.ts` declares `/determinus-refactor` as the stale
  proposal refresh command and `/determinus-coordinate` as the project/Epic audit,
...
- Severity: HIGH
  ...
  repository evidence and labels `repo_backed_fact`, `determinus_backed_fact`,
  ...
  command must not run `/determinus-refactor --execute` or mutate change state.
...
| --- | --- | --- | --- | --- |
| Change drift handoff | Coordinate detects repository-plan overlap but omits a refactor route. | `/determinus-refactor` performs bidirectional proposal/task reconciliation and validation. | DRIFTED | Surface evidence-backed change candidates with dry-run `/determinus-refactor <id>` handoffs. |
...
| Mutation boundary | Coordinate has no change-mutation path. | `plugin/src/consumer-integration.test.ts` locks the read-first/no-new-mutation boundary. | SOUND | Referrals only; execution stays in `/determinus-refactor` or approved existing Epic actions. |
...
`/determinus-refactor` for change artifacts and `/determinus-coordinate`'s typed Epic action
...
1. Add a **Refactor Coverage Audit** after `/determinus-coordinate` Phase 3/Phase 6.
2. A change candidate must be nonterminal, linked/participating, and supported
   by `repo_backed_fact` or `determinus_backed_fact` drift evidence. Output only
   `/determinus-refactor <change-id>` (dry-run default); never `--execute`.
3. An Epic candidate must name the exact stale field and retain the current
   approval-gated `determinus_epic_update`, `determinus_epic_reorder`, or convergence path.
...
- `.opencode/command/determinus-coordinate.md` — Phase 3 overlap outcomes; Phases
  7–9 report and approval actions.
- `.opencode/command/determinus-refactor.md` — stale-change reconciliation and
  dry-run/execute boundary.
- `determinus_spec show advance-epics` — `rq-epicCoordinateCommand01`,
