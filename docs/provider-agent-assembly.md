...
`determinus_INSTRUCTIONS.md` remains the full developer/reference protocol. It is not appended wholesale into the runtime agent; removed or compressed runtime protocol is tracked through `docs/determinus-runtime-protocol-coverage.md`, specs, tests, and command contracts.
...
2. **Scoped ADV protocol** — ADV runtime protocol is preserved in lean `adv.md` or covered by specs/tests/command contracts; `determinus_INSTRUCTIONS.md` is not global `opencode.json instructions[]` and is not appended wholesale into runtime `adv.md`.
...
5. **No generated provider agents** — `determinus-{provider}.md` files are retired and not compatibility aliases.
6. **No heuristic provider guessing** — unknown or missing provider/model identity emits no provider hint.
7. **Manual migration** — user-owned `agent.determinus-{provider}` config is cleaned up once by the user, not auto-migrated by sync.
...
- **2026-07-07 — `minimax` hint strengthened ([Sharper-Flow/Advance#196](https://github.com/Sharper-Flow/Advance/issues/196))**: The `minimax` provider hint now includes a verifiable-name rule that prevents fabrication of `/determinus-*` commands, `skill("…")` args, `Task(subagent_type="…")` args, and loadable file paths from context shape. Implementation follows MiniMax docs' three anti-hallucination patterns (permission to refuse, reference grounding, boundaries before generation) and aligns with peer-hint style (`gpt.md:7`, `kimi.md:6`). Sibling fix: [`JRedeker/toolbox#15`](https://github.com/JRedeker/toolbox/pull/15). Cross-provider `claude.md` gap tracked as fast-follow change `strengthenClaudeProviderHint`.
...
3. Keeps `determinus_INSTRUCTIONS.md` absent from global `instructions[]`.
4. Removes stale generated `determinus-{provider}.md` files from global agents.
5. Removes retired concatenated provider prompt files `agent-parts/advance/determinus-{provider}.md` when present.
6. Does not write `agent.determinus-{provider}.prompt` refs.
...
| Metric | Meaning |
| --- | --- |
| `lean_determinus_runtime_prompt` | Lean canonical ADV runtime prompt without frontmatter |
| `determinus_reference_protocol` | Full `determinus_INSTRUCTIONS.md` reference protocol size, not embedded into runtime `adv.md` |
| `provider_hint` | Provider hint payload from standalone plugin when available |
| `determinus_dynamic_system_block_estimate` | Estimated ADV runtime banner/status additions appended to `output.system[0]` |
...
2. Remove `agent.determinus-claude`, `agent.determinus-gpt`, `agent.determinus-glm`, and `agent.determinus-kimi` entries.
3. Remove `agent.adv.disable` if it only existed to hide generic `adv` during provider-variant mode.
4. Confirm stale files are gone from `~/.config/opencode/agents/determinus-{provider}.md`.
...
| Symptom | Cause | Fix |
| --- | --- | --- |
| `determinus-claude` / `determinus-gpt` still appears selectable | Stale global agent file or config key remains | Remove `agent.determinus-{provider}` config, run `scripts/deploy-local.sh --fix`, restart OpenCode |
