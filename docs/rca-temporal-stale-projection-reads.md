...
degraded while `determinus_doctor` continuously reported the system fully healthy. The degradation
...
| # | Symptom | Evidence |
|---|---|---|
| 1 | Artifact hydration failed permanently | `determinus_change_show include:{agreement,design,artifactOnly:true}` returned the index with `readable: false` and `_artifactsError: "Temporal operation exceeded 1500ms timeout"` on every attempt, including solo calls with no parallel contention |
| 2 | Briefing-packet generation failed permanently | `_briefingPacketError: "Temporal operation exceeded 8000ms timeout"` |
| 3 | Writes reported timeout but landed | `determinus_task_update` returned `ToolExecutionTimeout` at 10000ms on nearly every call; re-reading always showed the mutation applied |
| 4 | Durable test evidence did not persist | `determinus_run_test` returned run IDs but durable records were absent, yielding `consumer_warnings: [{kind: "verification_missing"}]` for runs that genuinely executed and passed |
...
| 6 | Worktree deletion timed out | `determinus_worktree_delete` timed out at 8000ms on both dry-run and apply; cleanup completed with raw git |
...
The `determinus-designer` lane hard-requires an authoritative BRIEFING PACKET plus an
...
All frontend UI work in the change had to be rerouted to `determinus-engineer`. The work completed
...
`determinus-temporal-repair` engagements.

The acceptance readiness evaluator reads a **workflow-only projection that is not persisted
in the durable change snapshot**. `determinus_gate_status` surfaces this directly:
...
present in durable state (`determinus_task_show` showed attempt 3 with no `consumer_warnings`), and
...
|---|---|---|
| `contractReviewMatrixSetSignal` via `determinus_contract_review_matrix_set` | `ACCEPTANCE_REVIEW_MATRIX_MISSING` | **Cleared** between retries |
| report-submit + `determinus_verification_evidence_disposition` | `VERIFICATION_EVIDENCE_MISSING` | **Did not clear**, both returned `success: true` |
...
`determinus_doctor` reported 27+ tracked session queues active on the worker. Parallel Temporal
reads reliably contended: the same `determinus_task_show` call that timed out when issued alongside
...
`determinus_doctor` reported `healthy` throughout: `server_alive`, `worker_alive`,
...
- **Poisoned history** — `determinus_wip_state` returned `poisoned_workflows: []`.
- **Phantom / lost state** — change was real, `claim_inventory.completeness: "complete"`.
- **Unhealthy worker** — `determinus_doctor` healthy, and writes consistently landed.
...
- An entire agent lane (`determinus-designer`) was silently disabled by a degraded read path.
...
- A correctly applied remediation appeared to fail, driving a repair engagement that
  recommended an approval-gated `determinus_change_reenter` the user would have had to approve.
- Every write required a defensive verification read, roughly doubling tool calls.
- `determinus_doctor` gave false assurance throughout.
...
2. **Report "latest" resolution.** Determine whether it resolves per agent-lane. If it does,
   a stale error report from a lane that never ran (the `determinus-designer` attempt-1 error report
...
4. **Make degraded reads legible.** A caller receiving `readable: false` should be able to
   distinguish transient contention from hard failure. `determinus_doctor` should report read latency
...
5. **Decouple lane preconditions from fallible reads.** A degraded read path must not silently
   disable an agent lane. Either decouple the `determinus-designer` packet precondition, or give the
...
3. `determinus_doctor` surfaces read-latency and projection-staleness signals.
...
```
determinus_change_create target_path:/home/jon/dev/advance
...
- Three `determinus-temporal-repair` engagements; the third produced the projection-freeze diagnosis.
- Resolution that finally cleared the gate: re-firing
  `determinus_verification_evidence_disposition` after `determinus_doctor` confirmed worker health. The
