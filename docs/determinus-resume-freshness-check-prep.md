...
- **Target:** Resume path for ADV changes — `resume {id}`, `continue {id}`, `complete {id}`, session-reactivation, `/determinus-apply` mid-execution resume
...
### Reliability
- **HIGH** — `.opencode/agents/adv.md:189-200` (Step 1 intent table + Step 2 Load State): resume loads only the target change via `determinus_change_show` + `determinus_gate_status` + `_contextSnapshot`. No detection of codebase drift since `lastActivityAt`. *Impact: agent proceeds against stale API/code surface; tasks reference paths that may no longer exist.*
...
- **MEDIUM** — `determinus_change_create` rejects duplicate titles only at creation time (`.adv/specs/advance-workflow/spec.json:5509`). A change that becomes a duplicate later (because another active change narrowed onto the same scope) is never re-flagged on resume.
...
- **HIGH** — User must remember to manually invoke `/determinus-coordinate` (Epic-scoped), `/determinus-cleanup` (batch), or `/determinus-refactor` (batch/single) before resuming. None fires automatically on the default resume path. Each is also heavier than what a single-change resume needs.
...
### Code Quality
- **LOW** — Supersession logic is scattered across at least four surfaces: title-match in `determinus_change_create`, `superseded` close-reason in `determinus_change_close`, bucket logic in `/determinus-cleanup` (skills/determinus-cleanup/SKILL.md:40), and overlap audit in `/determinus-coordinate`. No unified resume-time synthesizer.
...
| Duplicate detection timing | spec.json:5509 — creation-time only | `/determinus-cleanup` bucket #1 normalizes across active+archived anytime (skills/determinus-cleanup/SKILL.md:40) | DRIFTED | Promote cleanup duplicate detection to resume-time advisory |
| Stale proposal reconciliation | `/determinus-refactor` manual invocation (skills/determinus-refactor/SKILL.md) | UntitledPhases/Ideate `stale_resume` fixture — deterministic on resume | DRIFTED | Add lightweight stale-resume advisory to default resume path |
...
| Repo freshness audit | `/determinus-coordinate` Phase 2-3 (`.opencode/command/determinus-coordinate.md:70-113`) — opt-in, Epic-scoped | Same logic needed at per-change resume | SOUND (logic exists, wrong entrypoint) | Reuse Phase 2-3 engine behind a lightweight resume entrypoint |
| Overlap taxonomy | `/determinus-coordinate` classifies `repo_backed_fact \| determinus_backed_fact \| judgment_call \| freshness_limited` | Same taxonomy fits resume findings | SOUND (reuse directly) | Inherit the four labels verbatim |
...
- `.adv/specs/advance-workflow/spec.json:5509` — duplicate detection fires only on `determinus_change_create`.

**What is correct (with source):**
- `/determinus-coordinate` already classifies findings into `repo_backed_fact | determinus_backed_fact | judgment_call | freshness_limited` (`.opencode/command/determinus-coordinate.md:101-108`) — a proven taxonomy to inherit.
- `/determinus-cleanup` already normalizes titles across active+archived (skills/determinus-cleanup/SKILL.md:40,46-52) — a proven duplicate detector.
- `/determinus-refactor` already runs drift scanners (EXACT/METADATA/FUZZY) against current code (skills/determinus-refactor/SKILL.md:79-86) — proven drift detection.
...
**Greenfield note:** If ADV were rebuilt today, the natural shape is one `resumeFreshness` field on `buildChangeContextSnapshot` carrying `{ lastActivityAgeMinutes, siblingOverlap[], archivedSince[], codebaseDrift, freshnessLabel }`, computed by a single resolver shared across `/determinus-coordinate`, `/determinus-cleanup`, `/determinus-refactor`, and the default resume path. The current scattered logic is the cost of growing these commands independently.
...
Additional signal: agent-session-resume explicitly preserves user deferrals ("skip", "park", "not now") across resume — ADV should consider the same for scope rejections recorded in `/determinus-refactor` intent-conflict resolution.
...
- `/determinus-coordinate` Phase 2-3 logic is the natural engine to reuse — its `repo_backed_fact | determinus_backed_fact | judgment_call | freshness_limited` taxonomy already exists and is spec-backed (`.adv/specs/advance-epics/spec.json:191`).
...
- Full agentic-sdlc-style artifact validation (task-graph cycle checks, least-privilege handoff, privacy boundary scans) — those are broader than the user's request. Worth a separate `/determinus-audit` engagement.
...
- `plugin/src/tools/change.ts:831-849` — `lastActivityAt` / `lastActivityAgeMinutes` already computed for `determinus_change_list`; reuse for the resume resolver.
- `.opencode/command/determinus-coordinate.md:70-113` — Phase 2-3 freshness + overlap audit logic to extract into a shared resolver.
...
1. **Cost ceiling**: how many tool calls is the resume-freshness pre-step allowed to cost before it violates the ADV resume latency budget? Conjecture: 3–5 (one `determinus_change_list` for siblings, one `determinus_change_list includeArchived:true` filtered by archived-since, one `git log --since <lastActivityAt> --oneline` for codebase drift, optional one `determinus_change_show` per sibling if capability overlap is non-trivial). Needs confirmation.
2. **Trigger threshold**: should the pre-step fire on every resume, or only when `lastActivityAgeMinutes` exceeds a band (e.g., >60min)? `/determinus-cleanup` uses 7d; `/determinus-refactor` uses hot-recency exclusion. Resume is more frequent than cleanup — likely needs a tighter band.
3. **Cross-project resume**: when a change has `target_path` / Epic membership spanning projects, does freshness check need to fan out to each target project? `/determinus-coordinate` already paginates `scope: "product"`; resume probably wants a narrower default.
...
5. **Mid-execution resume** (`/determinus-apply` after a session restart): does the same freshness check apply, or does execution-gate resume have a different cost/benefit profile because tasks already exist and may be partially complete?
...
- `.adv/specs/advance-epics/spec.json:191, 380-398` — `/determinus-coordinate` freshness law + Epic membership on resume
- `.opencode/command/determinus-coordinate.md:70-113` — Phase 2-3 freshness + overlap audit
- `skills/determinus-cleanup/SKILL.md:40, 46-52` — duplicate/superseded bucket detection
- `skills/determinus-refactor/SKILL.md:79-86` — drift scanner passes
- `.opencode/command/determinus-triage.md:107` — change↔change duplicate/supersession remains `/determinus-cleanup` ownership
