# Research Pack: determinus_run_test Alignment

Target: `determinus_run_test`
...
Analyze how `determinus_run_test` can better align with this repo's dev stack and ADV's agent model.
...
- System logs requested by user: user services, Temporal service warnings, OpenCode DB maintenance logs, test/determinus-related journal entries.
...
- Severity: MEDIUM
  ...
  - Follow-up: `/determinus-proposal Clarify determinus_run_test shell contract`
...
- Severity: HIGH
  ...
  - Follow-up: `/determinus-proposal Stream determinus_run_test output`
...
- Severity: HIGH
  ...
  - Evidence: TDD spec `rq-TDD008path.3` says red phase rejects `exitCode=0` and green phase rejects non-zero exit codes; docs still instruct `determinus_run_test phase:'red'` / `phase:'green'` in `determinus_INSTRUCTIONS.md:335-336` and `.opencode/command/determinus-apply.md:68-69,552-554`; current schema in `plugin/src/tools/test.ts:176-202` has no `phase`; `plugin/src/tools/test.test.ts:56-74` asserts no phase.
  ...
  - Follow-up: `/determinus-proposal Restore typed determinus_run_test phases`
...
- Severity: MEDIUM
  ...
  - Impact: Advance itself lacks the local throttle wrapper used elsewhere to avoid multi-session CPU saturation; `determinus_run_test` cannot recommend or detect stack-specific safe tiers.
  - Recommendation: add repo-local `bin/oc-test` wrapper first; then optionally teach `determinus_run_test` to surface a non-mutating advisory when it sees full-suite commands in repos with a wrapper.
  - Follow-up: `/determinus-proposal Add Advance oc-test wrapper`

### Observability

- Severity: MEDIUM
  - Evidence: `plugin/src/utils/metrics.ts:1-9` says metrics are in-memory and reset on plugin init; `plugin/src/tools/test.ts:261-302` records substep durations but not command duration/result details beyond current response; `journalctl --user --since '14 days ago' --grep 'determinus_run_test'` returned no entries; `journalctl --user -u temporal-dev --since '14 days ago' -p warning..alert` returned no entries.
  - Impact: operators can inspect current-session phase timing via `determinus_status`, but cannot correlate historical determinus_run_test timeouts, output-limit kills, or heavy-suite patterns after restart.
...
- Severity: LOW
  ...
  - Impact: not caused by `determinus_run_test`, but confirms local agent tooling can produce resource spikes; heavy test runs should be throttled and measured.
  - Recommendation: include resource-aware guidance in `determinus_run_test` output when commands look like full suites and no `timeoutMs`/wrapper is present.
  - Follow-up: `/determinus-task`

### Developer Experience

- Severity: HIGH
  - Evidence: `determinus_INSTRUCTIONS.md:284` says timeout responses include `errorClass: TestExecutionTimeout`; implementation returns `timedOut` and `timeoutMs` only (`plugin/src/tools/test.ts:294-302`).
...
- Severity: MEDIUM
  ...
  - Follow-up: `/determinus-proposal Add determinus_run_test evidence ledger`
...
- Severity: MEDIUM
  ...
  - Follow-up: `/determinus-proposal Add structured test evidence parsers`
...
| Observability | In-memory phase metrics only (`plugin/src/utils/metrics.ts:1-9,39-43`) | MCP tool guidance emphasizes actionable execution errors; current system logs have no `determinus_run_test` entries | DRIFTED | Return structured classifications and expose bounded health counters; avoid raw command/output logs. Greenfield: per-task evidence ledger with retention. |
...
3. nit — local-first AI quality agent.
   ...
   - Relevance: directly supports stack detection + framework-native tests; useful benchmark for `determinus_run_test` profiles/parsers.
...
1. Structured agent/tool-call testing.
   ...
   - Relevance: `determinus_run_test` should expose typed evidence suitable for exact assertions, not only text blobs.
...
2. Verified-spec-to-test loops for browser/UI work.
   ...
   - Relevance: less central to `determinus_run_test`, but supports future artifact handling for screenshots/traces and deterministic test artifacts.
...
- Restore/resolve typed TDD phase contract drift (`tdd-contract` + `.opencode/command/determinus-apply.md` + schema).
...
- `determinus_run_test` journal entries: none from `journalctl --user --since '14 days ago' --grep 'determinus_run_test'`.
...
- `lgrep_search_semantic` timed out twice while searching `determinus_run_test` evidence flow; analysis fell back to `lgrep_search_text`, direct `read`, specs, and docs.

## Open Questions for Research

1. Should `determinus_run_test` regain `phase` as part of the public schema, or should `tdd-contract` be amended to make phase purely narrative?
...
3. Should `determinus_run_test` expose profile inputs (`targeted|smoke|full`) while keeping `command` required, or should profiles be a separate tool?
...
- Apply docs: `determinus_INSTRUCTIONS.md:284,335-340`; `.opencode/command/determinus-apply.md:64-70,552-556`
...
- System logs scanned: `systemctl --user list-units --type=service --state=running --no-pager`; `journalctl --user --since '14 days ago' --grep 'determinus_run_test'`; `journalctl --user -u temporal-dev --since '14 days ago' -p warning..alert`; `journalctl --user -u opencode-db-maint.service --since '14 days ago'`
