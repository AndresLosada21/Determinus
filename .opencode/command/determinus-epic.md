...
Create/update an ADV Epic through a collaborative goal-first workflow. Epics are initiative containers; membership optional, order advisory. Epics are changes with `kind:"epic"` — they use the same `determinus_change_*` tools as regular changes, with epic-specific params.
...
- `/determinus-epic` — create or update an Epic initiative container (a change with `kind:"epic"`).
- `/determinus-proposal` — create a normal ADV change for one work item.
- `/determinus-triage` — coalesce overlapped changes↔issues and surface portfolio balance; does not create Epic state.
...
3. If the request is clearly one change rather than an initiative, recommend `/determinus-proposal` instead of forcing an Epic.
...
| Purpose                                 | Tool                |
| --------------------------------------- | ------------------- |
| List active Epics                       | `determinus_change_list` with `filter:{kind:"epic"}` |
| Inspect plausible overlapping Epics     | `determinus_change_show` (includes entries for epics) |
| Inspect active changes for related work | `determinus_change_list`   |
| Inspect portfolio balance when relevant | `determinus_change_list` + `determinus_change_show` |
...
| Option                  | Meaning                                                                                                                      |
| ----------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| update/clarify existing | Use `determinus_change_update` (with `link_change`/`unlink_change`/`reorder_entries` for entry ops) or `determinus_change_create` with `parent_epic_id` + `shell:true` to refine the existing Epic; run `determinus_change_show` to inspect entry convergence |
| merge duplicate         | No single merge tool exists; plan conflict-safe consolidation, then execute with `determinus_change_update` (`link_change` + `unlink_change`) only after explicit conflict dispositions |
| create new              | Create a distinct Epic because the ultimate goal or scope is different                                                       |
| stop                    | Do not mutate Epic state                                                                                                     |

× MUST NOT call `determinus_change_create` with `kind:"epic"` for a plausible duplicate until the user chooses `create new`.
...
- title
- `## Ultimate Goal`
- narrative context to pass in `determinus_change_create` with `kind:"epic"`
...
Ask for explicit final confirmation. `determinus_change_create` with `kind:"epic"` may run only after the `## Ultimate Goal` and final confirmation are both present.

## Phase 5: Mutate Through Change Tools

After confirmation:

1. Create new Epic with `determinus_change_create` (`kind:"epic"`), update existing Epic title/narrative with `determinus_change_update`, or refine entries with `determinus_change_create` (`parent_epic_id` + `shell:true`) and `determinus_change_update` (`link_change`). Run `determinus_change_show` to inspect entry convergence before escalating to mutating tools.
2. Add user-approved future work with `determinus_change_create` (`parent_epic_id` + `shell:true`).
3. Link user-approved existing changes with `determinus_change_update` (`link_change:"<changeId>"`).
4. Consolidate approved duplicate active Epics by moving/linking/unlinking entries with `determinus_change_update` (`link_change` + `unlink_change`) after plan review. No single merge tool exists.
5. Reorder entries with `determinus_change_update` (`reorder_entries:["id1","id2",...]`).
...
- `epic_owner_target_path` routes the **Epic owner project** when the Epic lives in another determinus-enabled project.
