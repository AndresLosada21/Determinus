...
**Produces:** Complete task graph via `determinus_task_add` (sole pre-impl task creator per rq-prep-out1), gap analysis (rq-prep-scope1), task sequencing with dependencies.
...
**Gate:** Completes `planning` only. `/determinus-task` is exempt (fast-track bundles proposal+discovery+design+planning).
<UserRequest>
$ARGUMENTS
</UserRequest>
...
Every `/determinus-prep` invocation must execute these steps:
...
- **No criteria firming** — acceptance criteria and success criteria belong in `/determinus-discover`; design-derived technical criteria belong in `/determinus-design`
...
`determinus_change_show changeId: <target> include: { snapshot: true, readyTasks: true }` collapses change + gate snapshot + ready-queue into one call. Add `include.ledger: true` only when picking up after a partial execution (re-entry path). When a fresh structured per-gate breakdown is needed (e.g. for prep methodology enforcement), fall back to `determinus_gate_status changeId: <target>`.
...
Then `determinus_spec action: "list"` + `determinus_spec action: "show"` for each affected capability.

**Epic context:** if the change has `epic_membership`, load compact Epic context with `determinus_change_show` (Epics include entries). Consider Epic entry order as advisory when sequencing tasks, but do not hard-block later tasks or gates because of order.

Stop if discovery or design gates are incomplete. `/determinus-prep` analyzes validated design decisions — it must not backfill pre-implementation gates.
...
If zero tasks AND design gate complete → synthesize task graph from design findings, deltas, and proposal context. If design gate pending → warn: run `/determinus-design` first.

### After Re-Entry

If this change was re-entered via `determinus_change_reenter`, the planning gate has been reset to `pending` and new tasks can be added via `determinus_task_add`. Existing tasks and completed work from prior execution are preserved. Run the full gap analysis against the expanded scope — treat previously completed tasks as evidence to validate, not as proof of acceptance. Add new tasks where the expanded scope introduces gaps.
...
5. **Cross-spec consistency** — `determinus_spec search` for conflicts, terminology inconsistencies
...
Fix gaps: `determinus_task_add` for missing tasks, `determinus_task_cancel` (with approval) for absorption/merges, document N/A for non-applicable concerns. Assign `metadata.tdd_intent` to every task.
...
- A valid non-test evidence route does not require `determinus_run_test` or red/green evidence. Select proof from the typed deliverable, `evidence_policy`, and `metadata.tdd_intent`.
...
- The apply task must create/update run state with `determinus_ops_run_upsert` before execution evidence is recorded.
...
- Evidence expectation: `determinus_ops_run_evidence_add` records bounded summaries and artifact pointers/rationales only; no credentials or raw production logs.
...
When `ChangeContract` exists, `/determinus-prep` must synthesize task refs alongside task graph decisions:
...
- Include a problem-statement projection excerpt via `determinus_change_show include.problemStatement`, or a proposal projection excerpt via `determinus_change_show include.proposal`, when task scope depends on the confirmed problem.
- Include agreement projection excerpts via `determinus_change_show include.agreement` in task verification notes when acceptance/constraints drive `contract_refs.verifies` or `contract_refs.respects`.
- Include design projection excerpts via `determinus_change_show include.design` for tasks implementing architecture-sensitive behavior.
- Read the acceptance projection via `determinus_change_show include.acceptance` when acceptance context is needed; do not create tasks that rely on manually editing it; acceptance proof is generated from typed contract review state.

### Delegation Hints

When creating tasks, `/determinus-prep` may set `metadata.delegation_hint` to signal execution routing:
...
If omitted, `/determinus-apply` determines routing from `tdd_intent`, title heuristics, and risk signals. See `determinus_INSTRUCTIONS.md § Delegation Routing`.

### Frontend Routing Metadata

When creating tasks whose owned scope is frontend/view/component UI work, `/determinus-prep` MUST set `metadata.frontend = "true"` to give `/determinus-apply` a structural classification signal. Classified UI work starts with `determinus-engineer` when delegated (or runs inline when risk-forced); successful same-cycle implementation then receives an `determinus-designer` follow-up. When the task scope is not frontend, omit the key (or set `"false"`).
...
- Keep `metadata.frontend` unset (or `"false"`) for backend logic, storage, APIs, persistence/state-store behavior, business rules, schemas, and infra tasks. `determinus-engineer` owns these.
...
`/determinus-apply` uses `metadata.frontend` as a structural classification signal after `metadata.delegation_hint` (Priority 1) and risk routing. It does not directly route initial implementation to `determinus-designer`; see `.opencode/command/determinus-apply.md § Delegation Routing` and `.opencode/agents/determinus-designer.md` for the engineer-first follow-up contract.
...
× MUST NOT route review/harden work to `determinus-designer` — `determinus-reviewer` remains the review/harden owner. When review scope includes design/frontend work, the orchestrator supplies a frontend/design skill or checklist anchor to the reviewer packet (see `.opencode/command/determinus-review.md` and `.opencode/command/determinus-harden.md`).
...
`determinus_change_show validate: true strict: true` → fix errors → re-validate. `determinus_gate_complete gateId: planning` → handle failure codes (`SCENARIO_MISSING`, `TASK_TDD_INVERSION`, `CROSS_REPO_MISSING_METADATA`).
...
From `determinus_task_list`: tasks atomic? cover all requirements? verification steps? dependencies explicit?
...
Action: `determinus_task_cancel` (with approval) → update parent description → redirect dependents.
...
For non-code tasks, use an `evidence_plan` with a valid `evidence_policy` instead of fake TDD. A valid non-test route does not require `determinus_run_test` or red/green. See **Non-Code Deliverable Evidence Policy** above.
...
`determinus_spec action: "search" query: <term>` → flag conflicts, terminology inconsistencies, overlapping scope.
...
**Check 4:** Cross-project coordination metadata — when change depends on or contributes to another determinus-enabled project, ensure tasks or change metadata identify `cross_project_links` and `external_dependencies` explicitly.
...
Won't items are findings too: document them in the proposal projection via `determinus_change_update proposal` AND route each to `determinus_change_create status: "backlog"` — the durable middle-tier option between folding scope into this change and losing the finding in prose.
...
║    After this: autonomous implementation via /determinus-apply          ║
║                                                                  ║
╚══════════════════════════════════════════════════════════════════╝
```
...
| Reply                    | Action                                                                                                                                           |
| ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| Tier A whitelist match   | Call `determinus_gate_complete gateId: 'planning' userApproved: true`, then begin `/determinus-apply` inline                                                   |
| `/determinus-apply {change-id}` | Counts as explicit approval. The agent completes planning with `userApproved: true` and proceeds to execution inline — no second approval prompt |
...
**Machine contract (CRITICAL):** when user replies with a Tier A whitelist word (or LLM classifies as `approve`), the agent MUST pass `userApproved: true` to `determinus_gate_complete gateId: 'planning'`. The machine contract enforced by `handlePlanningGateCompletion` is independent of the UX surface — inline approval is the upstream signal source.
...
- Missing tasks → `determinus_task_add` (with `blockedBy` if needed)
- Absorption/merge → `determinus_task_cancel` (with user approval) → update parent → redirect dependents
...
- Cross-spec conflicts → document resolution in the proposal projection via `determinus_change_update proposal`

---

## Phase 7: Progress Tracking

After EACH fix, keep progress in ADV state (`determinus_task_list`). Use `include.snapshot:true` on mutation tools or `determinus_change_show include.snapshot:true` to request `_contextSnapshot`. Emit no `CONTRACT STATUS` block.

---

## Phase 8: Validation

`determinus_change_show changeId: <target> validate: true strict: true` → fix errors → re-validate.

---

## Phase 8.5: Readiness Report

`determinus_gate_complete changeId: <target> gateId: planning userApproved: true`
...
`determinus_task_list` + `determinus_change_show validate: true` for target.
...
`determinus_gate_complete changeId: {change-id} gateId: planning userApproved: true` (no-op if Phase 8.5 already passed).
...
**Auto-continue:** After user approval, immediately begin `/determinus-apply` inline. This is the last human checkpoint before autonomous execution — user's "approve and continue" is the go-ahead to start implementation without any further confirmation.
...
| Add task               | `determinus_task_add`                                          |
| Cancel tasks           | `determinus_task_cancel` (requires user approval)              |
| List/show/search specs | `determinus_spec`                                              |
| Validate               | `determinus_change_show validate: true`                        |
| Planning gate          | `determinus_gate_complete gateId: planning userApproved: true` |
