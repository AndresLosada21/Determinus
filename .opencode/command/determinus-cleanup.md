...
> **CHECKLIST**: Default dry-run. Reversible closures require Tier B per-bucket approval (`rq-inlineApproval01.4`). Irreversible buckets require count-matched typed confirmation and reject `approve all` (`rq-cleanupHygieneScope01`). Deletion always delegates to the shared `determinus_worktree_delete` planner/executor or `determinus_worktree_cleanup` drain (`rq-terminalCleanupSafety01`). Never auto-archive; recommend `/determinus-archive {id}` to preserve per-change Tier B sign-off (`rq-inlineApproval01.3`).
...
`skill("determinus-cleanup")` → bucket rules, staleness heuristics, duplicate checks, approval parser, bulk-close procedure. If unavailable, use fallback below.
...
1. **Active changes** — `determinus_change_list({ sort: "stalest", excludeRecencyBands: ["hot"] })`. Record scanned count + hot-excluded count.
2. **Worktree drift** — `determinus_worktree_triage`.
3. **Merged archived branches** — `determinus_worktree_cleanup({ mode: "archived_branches", dryRun: true })`. Discovery only; `dryRun: true` is mandatory in this phase.
4. **State leaks** — `determinus_status({ view: "hygiene" })`.

> **DESTRUCTIVE DEFAULT WARNING:** Omitting `dryRun: true` here **DELETES branches instead of listing them**; `determinus_worktree_cleanup` has no safe default. Use the discovery call exactly as written. The apply form belongs only in Phase 5 after typed confirmation.
...
1. **Duplicate** — normalized title match or conservative ID suffix match → explicit per-candidate `determinus_change_close reason: superseded`.
...
4. **Ready to archive** — all gates except release done, all tasks done, no unresolved review findings → recommend `/determinus-archive {id}`.
5. **Healthy** — skip.

Inspect cheaply: duplicate pass over active+archived list; if hot/non-stale and not duplicate → Healthy; otherwise one `determinus_change_show` per stale change. Before closure buckets, check unarchived `fast_follow_of` children; move matches to blocked sub-bucket. If `--bucket`, retain only that bucket.
...
|---|---|---|---|---|
| 1 | Active-change debt | `determinus_change_list` + `determinus_change_show` | reversible | yes |
| 2 | Worktree drift | `determinus_worktree_triage` | **irreversible** | yes |
| 3 | Merged archived `change/*` branches | `determinus_worktree_cleanup mode: "archived_branches"` | **irreversible** | yes |
| 4 | Archived/closed state leaks | `determinus_status view: "hygiene"` | report-only | no |
...
The `determinus_worktree_triage` classes map to these groups as follows:

| `determinus_worktree_triage` class | Drift group | Deletion eligible |
|---|---|---|
...
Any unrecognized or new `determinus_worktree_triage` class MUST default to `needs-investigation` and is never eligible for deletion (fail-closed for unknown input).
...
`determinus_worktree_triage` output is advisory discovery and selection data — **never deletion authority** (`rq-terminalCleanupSafety01`). Every approved deletion first calls `determinus_worktree_delete dryRun:true` for the exact branch and records its returned `planToken`; apply must pass that token plus nonblank `approvalEvidence`. A legacy branch-only apply returns deterministic `PLAN_REQUIRED` and performs no shell or write. `determinus_worktree_delete` / `determinus_worktree_cleanup` independently re-verify terminal owning-change status, branch integration, clean worktree state, no live process CWD, and squash-merge-safe tree-SHA equivalence. Their refusals are final: surface each refusal verbatim, never retry around it.
...
- Ready → `→ Run /determinus-archive {id} to ship.`
...
For each approved candidate, call `determinus_change_close` with `approvedByUser: true`, `approvalEvidence`, the candidate `changeId`, reason, and `supersededBy` for Duplicate. For duplicate buckets, inspect each superseding target with `determinus_change_show` before closing.

Before Duplicate apply, `determinus_change_show` each `supersededBy` target. Missing target → skip only those candidates and report `skipped: missing supersededBy target`.

### Irreversible buckets

Worktree deletion → apply the exact returned `planToken` with `determinus_worktree_delete` per approved branch and the count-matched `approvalEvidence`. Branch deletion → `determinus_worktree_cleanup({ mode: "archived_branches", changeId: <approved candidate change ID>, approvalEvidence: <count-matched approval> })` without `dryRun`, once per approved candidate. Never issue an unscoped merged-branch cleanup apply: it would delete every currently eligible branch and bypass a subset approval.
...
|---|---|
| `/determinus-status` | Read-only overview; cleanup is actionable counterpart |
| `/determinus-refactor` | Refreshes stale proposal content; cleanup closes abandoned/dead proposals |
| `/determinus-archive` | Cleanup recommends only; archive owns Tier B sign-off |
...
| Purpose | Tool |
|---|---|
| Scan active changes | `determinus_change_list` |
| Inspect gates/tasks | `determinus_change_show` |
| Close approved candidates | `determinus_change_close` in a loop |
| Discover worktree drift | `determinus_worktree_triage` |
| DISCOVERY — merged archived branches (Phase 1; `dryRun: true`) | `determinus_worktree_cleanup` (`mode: "archived_branches"`, `dryRun: true`) |
| Discover state leaks | `determinus_status` (`view: "hygiene"`) |
| Delete a worktree | `determinus_worktree_delete` |
| APPLY — delete merged archived branches (Phase 5 only, after typed confirmation) | `determinus_worktree_cleanup` (`mode: "archived_branches"`) |
| RECOVERY — a prior cleanup timed out during discovery | `determinus_worktree_cleanup` (`skipDiscovery: true`) — drains already-queued pending deletes without repeating the discovery scan. Worktrees mode only; ignored in `archived_branches`. Drains only entries a prior discovery already validated, so it grants no new deletion authority (`rq-terminalCleanupSafety01`). |
