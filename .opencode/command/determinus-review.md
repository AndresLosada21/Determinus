...
2. If empty → `determinus_change_list` → auto-select or `question` tool

## Directive Read Protocol
Resolve `{id}` from Target Resolution, then read the typed phase plan:

```text
determinus_change_show changeId: {id} include: { phasePlan: true }
```
