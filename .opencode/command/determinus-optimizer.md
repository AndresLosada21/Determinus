...
Find bloated, overly verbose, overly complex, or under-optimized code and synthesize an evidence-backed simplification proposal. This command is read-only and does not replace `/determinus-slop-scan`; slop-scan owns detector coverage and smell reporting, while optimizer owns proposal synthesis for long-term maintainability.
...
**× MUST NOT:** perform code edits, ADV state mutation, agenda creation, task creation, or automatic deletion. × MUST NOT auto-delete code based on heuristic or sub-agent judgment. × MUST NOT invoke `/determinus-*` slash commands from worker prompts.
...
Use bounded first-level `explore` scanners for independent simplification perspectives. Add `determinus-researcher` only when external best-practice/reference evidence is needed for a recommendation.
...
- Scanner workers must NOT invoke any `/determinus-*` slash commands.
...
## Next ADV Command
{`/determinus-proposal <summary>` or `/determinus-task` with rationale}
```
...
- Does not replace `/determinus-slop-scan`; slop detector coverage remains slop-scan-owned.
