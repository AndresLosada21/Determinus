...
**× MUST NOT:** create change, create tasks, complete gates, or silently commit to implementation design. This command does not call `determinus_change_create`, `determinus_gate_complete`, or `determinus_task_add` directly.
...
| Exit                    | Condition                                                                 |
| ----------------------- | --------------------------------------------------------------------------- |
| ✅ one-change → `/determinus-proposal` | Idea is focused enough to fit a single change                             |
| ✅ initiative → `/determinus-epic`     | Idea is initiative-sized (see Phase 4) and should be routed to `/determinus-epic` |
...
Invoke `determinus-researcher` to validate the idea against architecture, patterns, simplicity, and security dimensions. Carry `validation.status` through to the summary:
...
If initiative-sized, recommend `/determinus-epic` as the exit path. Otherwise, treat it as a one-change idea and recommend `/determinus-proposal`.

## Phase 5: Exit

Propose exactly one of the four exit paths:

- **one-change → `/determinus-proposal`** — focused enough for a single change
- **initiative → `/determinus-epic`** — initiative-sized; Epic-sized work should route to `/determinus-epic`
- **iterate** — keep exploring; list the next 1–3 concrete questions
- **stop** — user chooses not to pursue

The exit is a handoff recommendation only. `/determinus-idea` does not create changes, tasks, gates, or Epics directly.
...
- × silent state mutation — Phase 5 exits that invoke `determinus_change_create`, `determinus_gate_complete`, or `determinus_task_add` directly
