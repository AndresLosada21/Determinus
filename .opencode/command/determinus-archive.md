...
If empty → `determinus_change_list` → auto-select the only plausible change; ask via `question` only if multiple plausible targets remain.

---

## Phase 1: Pre-Archive Checks

1. `determinus_change_show changeId: {id} include: { executiveSummary: true, subagentReports: true }` → verify status "active", load executive summary, Release Readiness Summary, and harden evidence for sign-off report
   - If the returned change has `epic_membership`, load compact parent context with `determinus_change_show` (Epics include entries) before archive execution. Record Epic ID, entry ID/title/order, and member status when available.
...
2. `determinus_task_list` → all tasks must be "done". If incomplete → ARCHIVE BLOCKED banner → stop
3. `determinus_change_show validate: true strict: true` → if fails → show errors/warnings → stop and review the validation output before retrying
4. `determinus_status` → check for `[doctor]` entries: JSON/SQLite inconsistency or broken refs → block; pending WAL → warn only (advisory — benign when transient, escalate only if it persists after rerunning `/determinus-status` or restarting OpenCode)
...
On pass, plan generation of `CONTRACT_TRACEABILITY.md` in archive bundle with contract item IDs, task refs, matrix status, evidence summary, and amendment audit. On failure, stop before Phase 2 and direct user to `/determinus-review {change-id}` or contract re-entry/amendment.
...
`determinus_gate_status` → display all 7 gates. If any incomplete before `release` → stop with guidance.
...
### Executive Summary
{Read via determinus_change_show include: { executiveSummary: true, subagentReports: true }; source from _executiveSummary, including Release Readiness Summary when /determinus-harden appended it. Preserve outcome, value/why it matters, verification, risks/follow-ups, and supporting evidence for the non-technical release-approval reader. Technical terms may appear as parenthetical supporting detail. The artifact is persisted by /determinus-review Phase 7 at acceptance time and enriched by /determinus-harden before archive. If missing at sign-off, stop and surface the gap — do not recompose.}
...
| `dry run` / `dryrun`                                                           | Run `determinus_change_archive dryRun: true`, present results, re-prompt with same options                                         |
...
For each affected capability: `determinus_spec action: "show"` → verify new requirements present. Verify archive directory exists with change.json and ARCHIVE_SUMMARY.md.

Epic child archive verification:

- If the change has no `epic_membership`, record `Epic: n/a` for the final report.
- If the change has `epic_membership`, reload the parent Epic with `determinus_change_show` (Epics include entries) after `determinus_change_archive phase9: "run"` returns shipped/merged release proof.
...
- If the Epic entry/member projection is `projection_stale`, `projection_pending`, `target_unreachable`, or otherwise stale, run `determinus_change_show` to load the Epic including entries and trigger supported bounded convergence. For already-archived children whose entries still appear active, use `determinus_change_show` to backfill `terminal_summary` from canonical child/archive evidence so Epic progress can recompute. Do not read or edit ADV state files directly.
...
- `determinus_reflect changeId: {change-id}` → produce the reflection report
...
- If reflection fails → record `Reflection: failed: <reason>; nonblocking`, emit `[ADV:WARN] Reflection generation failed: {reason}; does not block release. Rerun /determinus-reflect {change-id} later.`
...
- Epic: {n/a | {epic_id}/{entry_id} terminal state verified | terminal projection recorded | membership converged via determinus_change_show | warning: <reason>}
...
`determinus_change_archive`, it passes `phase9: "run"` with `worktreePath` so the
...
1. **Active change overlap** — `determinus_change_list` → if other active changes touch same files or same local subsystem → mark `overlap-risk`
...
4. If main is dirty (non-ignored changes present):
   - ADV commits all non-ignored tracked and untracked changes: `git -C "$MAIN" add -A && git -C "$MAIN" commit -m "chore(determinus-archive): checkpoint main before archiving {change-id}"`.
...
When `git rebase` surfaces conflicts during Phase 9, /determinus-archive runs the full classification + resolution loop:
...
- `hook_strategy == "none"` AND change touched any path in deployed asset list (`.opencode/`, `determinus_INSTRUCTIONS.md`, `skills/`, `plugin/src/`, `scripts/deploy-local.sh`) AND `$MAIN/scripts/deploy-local.sh` exists AND is executable → Step 5.0 MUST already have run; record `sync_action: manual fix`
...
- First call `determinus_worktree_delete branch: "change/{change-id}" dryRun: true` and retain the typed `planToken` for that exact target.
- Apply with `determinus_worktree_delete branch: "change/{change-id}" planToken: "{planToken}" approvalEvidence: "archive sign-off for change {change-id}"`.
...
Idempotency: already-cleaned entries skip via `determinus_worktree_delete` record check.
...
`determinus_worktree_delete` unavailable globally → emit one info banner with manual fallback.
...
`determinus_change_archive` defaults to closing linked GH issues after durable archive transition. Agent does not run `gh issue close` manually.
...
3. **Branch on the terminal result.**
   ...
   - **Non-terminal** (timeout / blocked / red-CI-unresolved / missing creds / API error):
     a. Render `Pending auto-merge.` with `finalization.prUrl` and the exact retry command on its own line: `determinus_change_archive changeId:{change-id} worktreePath:{worktree} phase9:"run"`.
...
5. **Completion fallthrough.** If the Task tool spawn is unavailable (no `determinus-ci-waiter` runtime), render `Pending auto-merge.` + retry command — status quo behavior, non-regressive. The change stays active; the human re-invokes archive or runs `bin/adv doctor` to re-drive the archived-but-unmerged branch when ready.
...
| Purpose | Tool                                                                            |
| ------- | ------------------------------------------------------------------------------- |
| Archive | `determinus_change_archive changeId: <id> worktreePath: <worktree-root> phase9: "run"` |
