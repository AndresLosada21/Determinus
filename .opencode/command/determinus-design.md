...
- `determinus_change_show include: { proposal: true, agreement: true }`
...
- **Epic context:** if the change has `epic_membership`, load compact Epic context with `determinus_change_show` (Epics include entries) and consider the Epic narrative/entry order when sequencing design decisions. Epic order is advisory; do not let it block design gate completion.

If agreement is missing or not approved, stop and complete `/determinus-discover` first.
...
7. **Risks and mitigations** — a risk that a finding or concern might be lost MUST be framed as a durable backlog-status change (route it to `determinus_change_create status: "backlog"`), never as `no change owns it`; an unrecorded risk is a dropped finding.
...
- If design invalidates an approved AC/SC, treat discovery re-entry as routine: call `determinus_change_reenter fromGate: "discovery"` with the criteria delta, then rerun discovery/design/prep. Do not silently rewrite agreement criteria in design.
...
Keep the design actionable for `/determinus-prep`; it should explain why the plan is correct, not what files exist.
...
When the design involves module shape, interface design, seam placement, or testability, load `skill("determinus-codebase-design")` for the shared architecture vocabulary (module, interface, depth, seam, adapter, leverage, locality), the deletion test, and the dependency categories. Use its terms exactly.
...
Persist design content via `determinus_change_update design` (`change.documents.design`).
...
- Spawn the independent validator agent (`determinus-researcher`) with a validator-specific prompt. This step is mandatory — it must run before Phase 4. If task tool is unavailable, skip gracefully and record `INCONCLUSIVE` via `determinus_change_update design` appended to the design projection (see Phase 3.6).

**Validator input:** design artifact content + generated researcher-lane briefing packet (`determinus_change_show include: { briefingPacket: true, briefingPacketLane: "researcher" }`). Inject `_briefingPacket` for agreement context; do not reconstruct objectives, AC, constraints, avoidances, affected_files, or epic_context manually. Do not ask validator to read `artifacts.*.path`.
...
3. SPEC-LAW COMPLIANCE — Does this design contradict any existing spec requirement? Use determinus_spec to check. → maps to architecture_judgement.spec_law_implications
...
EXPECTED OUTPUT: call determinus_subagent_report_submit with RESEARCHER_REPORT per .opencode/agents/determinus-researcher.md (architecture_judgement is required).
```
...
Record the validation result via `determinus_change_update design` as a compact summary appended to the design projection.
...
- optional visual comparison block when side-by-side design alternatives are easier to judge than prose alone. Load `skill("determinus-user-intuit")` for the structured comparison presentation protocol if skill is available; otherwise continue with existing inline comparison workflow
...
- Real user-value tradeoffs → emit **Inline Approval prompt (Tier A)** before `/determinus-prep`.
...
- Straightforward design + no tradeoff + no unresolved `CONFLICT` + no contract-compromise risk + validator `pass`/`caution`/`unknown` → proceed to `/determinus-prep`; no pause.
...
| Reply                                                                     | Action                                                              |
| ------------------------------------------------------------------------- | ------------------------------------------------------------------- |
| Tier A whitelist match (continue, go, approve, looks good, proceed, etc.) | Begin `/determinus-prep` inline                                            |
| `/determinus-prep {change-id}`                                                   | No-op; OpenCode dispatches                                          |
| `/determinus-discover {change-id}` or `revisit discovery`                        | Halt design; user re-enters discovery                               |
| Free-form text                                                            | Treat as design revision; apply via `determinus_change_update`, re-present |
...
Trigger: after reading agreement via `determinus_change_show include.agreement`, the chosen design path can only work by violating its acceptance criteria, explicit constraints, or stated avoidances. Surface risk; do not silently proceed.
...
Reply:
- `keep with compromise` — accept and amend `change.documents.agreement` via `determinus_change_update agreement`, then proceed to /determinus-prep
- `revise` (or describe the alternative) — agent finds an alternative path preserving all constraints
- `revisit discovery` (or `/determinus-discover {change-id}`) — re-enter discovery to renegotiate scope or objectives
...
| Reply                                                           | Action                                                                    |
| --------------------------------------------------------------- | ------------------------------------------------------------------------- |
| `keep with compromise` (or whitelist + explicit acknowledgment) | Amend `change.documents.agreement` via `determinus_change_update agreement` (append "Design Compromise" section), then `/determinus-prep` |
| `revise` or revision text                                       | Find alternative design preserving constraints; re-present                |
| `revisit discovery` or `/determinus-discover {change-id}`              | Halt design; user re-enters discovery                                     |
| `defer` / `cancel` / `stop`                                     | Halt change; record reason in change notes                                |

**Amendment procedure for "keep with compromise":**

- `determinus_change_update agreement` → append `Design Compromise` section to `change.documents.agreement`.
- Document compromised item, why unavoidable, approval evidence (reply text + timestamp).
- Proceed to `/determinus-prep` only after persisted.

### Phase 4.5: Persist Revisions

If user requests adjustments, update the design/proposal artifacts via `determinus_change_update`.
...
`determinus_gate_complete changeId: {change-id} gateId: design`
...
> design ✓ → planning
>
> → `/determinus-prep {change-id}`
```

**Auto-continue:** After gate completion, begin `/determinus-prep` inline. Covers explicit approval and clean auto-pass. Do not ask "shall I proceed?" Approval/auto-pass is go-ahead.
