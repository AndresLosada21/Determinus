...
Orchestrate architecture inconsistency detection. The `determinus-arch-detection` skill owns the three detection passes: pass 1 (deterministic tools for known stacks) → pass 2 (research fallback for unknown stacks) → pass 3 (AI heuristic as universal fallback).
...
---
## Phase 0: Load Skill
`skill("determinus-arch-detection")` → three-phase detection strategy, Known-Stack Rule Matrix, Research Fallback, finding format, severity rubric, P33 boundaries.
...
---
## Write Metadata
After report generation, call `determinus_project_metadata action:"write"` with:
- `key`: `"arch-scan"`
...
Persists the scan result for display in `/determinus-status`.
