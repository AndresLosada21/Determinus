...
Orchestrate AI-code quality scan. Methodology lives in `determinus-slop-detection`; command owns args, pre-flight, dispatch, aggregation, metadata.
...
`skill("determinus-slop-detection")` → two-phase detection strategy, smell categories, thresholds, confidence, report schema. Required — if the skill fails to load, stop and report a broken deploy (run scripts/deploy-local.sh --fix).
## Pre-flight
...
Also include smell definitions for category, file list, novelty check, and these bans: do all work inline; do NOT spawn sub-agents/delegates; do NOT invoke `/determinus-*`.
...
- `determinus_DEBUG=1`: raw sub-agent prompts/responses to stderr, pattern context.
## Execution
