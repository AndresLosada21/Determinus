...
2. If empty → `determinus_change_list` → auto-select the only plausible change; ask via `question` only if multiple plausible targets remain
3. If none exist → stop and suggest `/determinus-proposal`
...
Every `/determinus-discover` invocation must execute these 9 protocol steps and emit a Discovery Checklist section summarizing their results:
...
| 2   | **Prior Research Extension**                       | Extends                   | Cited artifacts (including `/determinus-improve` research packs under `docs/*-prep.md`) + ≥1 new finding (or "No prior research found") |
| 3   | **Conflict & Related-Work Scan** (Phase 1.6)       | Conflict Scan             | Results from `determinus_change_list` (includeArchived), `determinus_change_show validate: true`, and typed change inventory with Epic/member context     |
...
- **No architecture decisions** — those belong in `/determinus-design`

---

## Phase 1: Load Context

- `determinus_change_show changeId: <target> include: { snapshot: true }` — single call returns change + rendered gate snapshot (replaces separate `determinus_gate_status` round trip)
- `determinus_spec action: "list"` and `determinus_spec action: "show"` for affected capabilities
...
If proposal gate is still pending → stop and direct user to `/determinus-proposal` first.
...
If `determinus_change_show` reveals a `cross_project_origin` field:
...
If `determinus_change_show` reveals a `fast_follow_of` field:

1. **Confirm the parent exists** — call `determinus_change_show changeId: {parent_change_id}` to validate the parent change exists in current project (archived or closed parents are valid)
...
If `determinus_change_show` reveals `epic_membership`:
...
Execute skill discovery protocol from `determinus_INSTRUCTIONS.md § Skill Discovery Protocol`, then check for skill gaps and pending reviews.
...
If no matching skill was found for a domain clearly relevant to change's **core problem** (not tangential), the agent MAY create a skill on demand. See `determinus_INSTRUCTIONS.md § Skill Creation Protocol` for the full trigger conditions, naming convention, assembly template, and creation flow.
...
1. `determinus_change_list includeArchived: true` → enumerate all changes (active + archived) with Epic membership context
2. `determinus_change_show validate: true` on target change → note that own-change pre-prep warnings (NO_TASKS, NO_DELTAS) are expected and should NOT be reported as conflicts
3. Build a complete paginated typed change inventory with explicit completeness state:
   ...
   - **Archived changes** are related context only (inspect with `determinus_change_show` for prior work that may inform or constrain the current proposal)
...
For relevant archived changes, use `determinus_change_show` to inspect their tasks and decisions. Prior work may inform or constrain current proposal.
...
- The existing `rq-disc-tax2` trigger (see Phase 2.5) halts discovery and hands off to `/determinus-clarify`.
- **No new halt machinery** — the block reuses the existing CRITICAL→halt→`/determinus-clarify` path.
...
Run a structured ambiguity scan using the taxonomy from `determinus_INSTRUCTIONS.md § Ambiguity Taxonomy`:
...
**Finding shape** (from `determinus_INSTRUCTIONS.md § Ambiguity Taxonomy`):
...
- `docs/*-prep.md` — research packs (including `/determinus-improve` output). These contain pre-vetted competitors, alternatives, and emerging patterns that MUST be cited before running any new external searches on same domain.
- Archived changes — `determinus_change_list includeArchived: true` → inspect relevant archives

**Rules:**

- Cite each found artifact in the "Extends" section
- For `/determinus-improve`-style research packs, explicitly cite the `Competitors & Alternatives`, `Emerging Patterns`, and `Applicability to This Repo` sections when they are relevant to the discovery's open design questions
...
- No prior artifacts → report "No prior research found" (non-blocking). If the discovery agenda includes ecosystem unknowns or viable external alternatives, note that running `/determinus-improve {target}` first would produce a reusable research pack.
...
If 2+ viable approaches have user-value tradeoffs, run the inline Tradeoff Prioritizer Protocol from `determinus_INSTRUCTIONS.md` and produce criteria-based comparison before asking the user. If only one viable approach remains after evidence, record why.
...
3. Emit findings inline in the LBP Check section. Recommend `/determinus-improve {target}` as a follow-up to persist the findings as a durable research pack when the discovery agenda will need them repeatedly.
...
2. **Resolution log check** — read `## Clarify Resolution Log` from proposal content via `determinus_change_show include.proposal` if present; previously-resolved findings are excluded from current trigger count
3. **Classify output noise:**

| Class                  | Condition              | Action                                                                                                                                           |
| ---------------------- | ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Blocking ambiguity** | CRITICAL ≥ 1           | Halt discovery. Do NOT call `determinus_gate_complete gateId: 'discovery'`. Output evidence quotes and hand off to `/determinus-clarify {change-id}`.          |
...
5. **Rerun cap:** After `/determinus-clarify` resolves findings and user reruns `/determinus-discover`, cap at 2 reruns before escalating to user via `question` tool per EC4
...
- Use `determinus_change_update` to refine proposal content
...
`skill("determinus-opportunity-scout")` → bounded scout methodology (modes, output schema, routing taxonomy, prompt templates, opt-out, degradation). Use `discovery` mode for this phase. If skill is unavailable, the scout phase is inconclusive and recorded in the phase output; do not auto-adopt or surface candidates without the skill's routing taxonomy.

When the change involves module shape, interface design, seam placement, or testability, load `skill("determinus-codebase-design")` for the shared architecture vocabulary (module, interface, depth, seam, adapter, leverage, locality), the deletion test, and the dependency categories. Use its terms exactly.
...
Generate the researcher-lane briefing packet via `determinus_change_show include: { briefingPacket: true, briefingPacketLane: "researcher" }` and inject `_briefingPacket` into the scout prompt. Do not reconstruct `scope`, `contract`, `affected_files`, or `epic_context` manually.

Inject into the `determinus-researcher` scout prompt:
...
EXPECTED OUTPUT: return ScoutCandidate rows and call determinus_subagent_report_submit with RESEARCHER_REPORT per .opencode/agents/determinus-researcher.md
```
...
- Deferred questions carry forward as constraints for `/determinus-design`
...
3. Emit the **Inline Approval prompt (Tier A with `/determinus-clarify` literal detection)** per `docs/command-voice-standard.md` § Inline Approval Voice:
   ...
   The literal `/determinus-clarify {change-id}` route remains available as a halt path; reply detection below is unchanged.

4. **Reply detection rules (in order):**

   | Reply                                                  | Action                                                                                                                                                           |
   | ------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
   | Trimmed = `/determinus-clarify` or `/determinus-clarify {change-id}` | Halt cleanly: no agreement projection write, no `determinus_gate_complete`, return control to user with instruction to rerun `/determinus-discover {change-id}` after `/determinus-clarify` |
   | Trimmed first token = `/determinus-clarify`                   | Same halt branch                                                                                                                                                 |
   ...
   **× Do NOT** treat phrases like "I want to clarify something" or "let's clarify X" as `/determinus-clarify` invocation. Only the literal slash-command form triggers the halt branch. Non-literal "clarify" intent is revision text (per `rq-disc12.2`).

> **Note:** The `/determinus-clarify` halt path at Phase 4.5.1 is same mechanism as the Phase 2.5 trigger evaluation halt. Both hand off to `/determinus-clarify` with same rerun instruction. Phase 2.5 catches ambiguity findings in proposal scope/User Outcomes; Phase 4.5.1 catches ambiguity introduced by criteria revision text.

5. If revised criteria still need substantial clarification after 3 re-runs, recommend the `/determinus-clarify` branch instead of continuing to loop.
6. If criteria are empty or weak, keep the approve option but mark the `/determinus-clarify` branch as the recommended path.
...
**× MUST NOT:** Complete `discovery` gate without criteria approval. Do not invoke `/determinus-clarify` directly outside this checkpoint outcome — pause and hand off to user.

### Phase 4.5.2: Structured Criterion Authoring Guidance (rq-disc16)

<!-- rq-disc16 -->

`/determinus-discover` produces four parse-safe criterion shapes. Each shape is minted into a stable `ACn`/`SCn`/`Cn`/`DONTn` contract item with an optional typed variant. The variant is advisory for presentation and review; the canonical `text`, stable `id`, `kind`, and `evidencePolicy` remain the contract authority.
...
Once criteria are approved at Phase 4.5.1 and all open questions are resolved (or explicitly deferred), persist agreement content via `determinus_change_update agreement` (`change.documents.agreement`). The Phase 4.5.1 inline approval is the sign-off — no additional `question` tool prompt.
...
If `determinus_gate_complete changeId: {change-id} gateId: discovery` returns `DISCOVERY_CONTRACT_MISSING`: fix parser/schema failures in approved agreement and retry gate; contract derivation is internal to the acceptance pipeline.

---

## Phase 5: Complete Gate

`determinus_gate_complete changeId: {change-id} gateId: discovery`
...
- Open design questions for /determinus-design
...
> discovery ✓ → design
>
> → `/determinus-design {change-id}`
```

**Auto-continue:** After user approves AC + agreement, begin `/determinus-design` inline. Do not ask "shall I proceed?" Approval is go-ahead.
