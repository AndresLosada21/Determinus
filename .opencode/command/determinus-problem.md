...
**Persistence:** Conversational triage; RCA carries forward via `## Root Cause Analysis` in the proposal projection (`change.documents.proposal`) through `determinus_change_update proposal` when defect path taken, otherwise ephemeral.

**× MUST NOT:** create change, create tasks, complete gates, or silently turn triage into fix implementation. This command does not call `determinus_change_create`, `determinus_gate_complete`, or `determinus_task_add` directly.
...
| /determinus-proposal | Scope looks larger, riskier, more systemic, or any direct-fix guardrail fails |
...
Dispatch `determinus-researcher` when the symptom touches a runtime, library, framework, API, or ecosystem surface. Carry `validation.status` through to the summary:
...
- **Spec-law change required** — route to `/determinus-proposal` with a draft spec-delta obligation.
...
- **Uncertain** — uncertain spec-law impact MUST NOT be direct-fix; route to `/determinus-proposal` or keep investigating until impact is clear.

`/determinus-problem` remains read-only: it MUST NOT create changes, tasks, gates, or spec deltas directly.
...
If any guardrail fails, the next step is `/determinus-proposal`, not direct fix.
...
- **/determinus-proposal** — scope is larger, riskier, systemic, any guardrail fails, or spec-law impact is uncertain/change-required
...
- Root cause (if defect origin) — when the issue is a defect/bug/regression, attach a Root Cause Analysis (RCA) section that downstream `/determinus-proposal` or `/determinus-task` invocations MUST carry forward (per rq-defectOriginRca01). RCA shape reuses the bullets above; if `/determinus-problem` already ran, its output IS the RCA.

## Anti-Patterns

- × no sub-agent dispatch — skipping Tier 1 or Tier 2 and synthesizing without codebase or ecosystem context
- × silent state mutation — Phase 7 exits that invoke `determinus_change_create`, `determinus_gate_complete`, or `determinus_task_add` directly
- × opaque scope decisions — Phase 6 triage classify without surfacing the guardrails
- × unjustified direct-fix recommendation — recommending direct-fix when any guardrail fails; route to `/determinus-proposal` instead
