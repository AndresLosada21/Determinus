...
`skill("determinus-audit")` → quality gates, audit dimensions, sub-agent roles, drift severity, report schema. Required — if the skill fails to load, stop and report a broken deploy (run scripts/deploy-local.sh --fix).
...
1. `determinus_spec action: "list"` → stop if no specs.
2. `determinus_change_list` → warn active changes may affect accuracy.
...
> Ambiguity findings can be resolved via `/determinus-clarify`. Pass the structured findings list as context. Resolution writes back to the relevant spec file (not ADV change state).

---

## Phase 5: Write Metadata

After successful completion, call `determinus_project_metadata action:"write"`:

- `key`: `"determinus-audit"`
...
| List changes | `determinus_change_list` |
