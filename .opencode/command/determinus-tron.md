...
---
## Phase 1: Load Skill
`skill("determinus-tron")` → provides investigation protocol, search priorities, evidence requirements, report schema.
...
1. `determinus_project_context` + `determinus_change_list`
...
This command declares `agent: determinus-tron` in frontmatter, so OpenCode invokes the `determinus-tron` subagent directly — no Task-tool spawn. ADV orchestrator agents deny `task: determinus-tron`, making `/determinus-tron` the only entry point. The agent's system prompt carries behavioral instructions; establish this packet plus mode-specific context as the working scope:
...
OPTIMIZATION_CANDIDATES (optional):
  - {validated static opt-scan candidates; if present, Tron preserves them as read-only optimization_candidates with detector_id, source evidence, expected_cost_shape, false_positive_caveat, verification_needed, and a recommended `/determinus-optimizer` handoff}
EXPECTED OUTPUT: return TRON RECONNAISSANCE REPORT and call determinus_subagent_report_submit with TRON_REPORT per .opencode/agents/determinus-tron.md when CHANGE is a real ADV change
```
...
**Opt-scan candidates:** If validated static opt-scan candidates are available, pass them in the spawn packet under `OPTIMIZATION_CANDIDATES`. Tron consumes them read-only, preserves all source evidence, and includes them as advisory `optimization_candidates` in the `TRON_REPORT` with a recommended `/determinus-optimizer` handoff. Do not run opt-scan automatically from this command if candidates are not already available; candidate generation is outside Tron's scope.
## Key Tools
| Purpose | Tool |
|---------|------|
| Skill | `skill("determinus-tron")` |
| Context | `determinus_project_context`, `determinus_change_list` |
| Structure | lgrep file tree, lgrep repo outline |
| Invocation | Command frontmatter `agent: determinus-tron` (direct subagent invocation; not Task-spawned) |
