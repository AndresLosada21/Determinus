...
Cross-repo tasks: respect `scope_repos`; record `origin_repo_id`, `origin_repo_project_id`, `origin_repo_path`, `product_id`. Cancellation: use `determinus_task_cancel` with explicit user approval. `determinus_task_update status: cancelled` is rejected.
...
| `determinus_task_update status: cancelled` | `determinus_task_cancel` with user approval |
...
2. If empty → `determinus_change_list` → confirm/select via `question` tool
3. If none → suggest `/determinus-proposal`

## Gate Prerequisite Check

`determinus_gate_status changeId: {change-id}`
...
× `/determinus-apply` MUST NOT complete discovery, design, or planning gates.
...
- `determinus_INSTRUCTIONS.md § Context Freshness`
- `determinus_INSTRUCTIONS.md § TDD Protocol (RSTC)`
- `determinus_INSTRUCTIONS.md § Doom Loop Detection`
- `docs/apply-subagent-packets.md` for sub-agent packet templates

`determinus_run_test phase` is descriptive metadata, not gate enforcement. Use `passed`, `classification`, `exitCode` as evidence. Inline TDD completions include `lastRedRunId` and `lastGreenRunId` from `determinus_run_test` calls. When a task's `evidence_plan` selects a non-test route, record `proof_target`, bounded rationale, and `review_conclusion` before completion.
...
3. After reentry with `determinus_change_reenter`, walk reopened gates, then resume `/determinus-apply` — new tasks will be available alongside existing completed work.

### Finding Routing

Out-of-scope findings surfaced mid-lifecycle MUST be routed to a durable backlog-status change with `determinus_change_create status: "backlog"` — not reflexive active-change creation, not prose-only notes. Route the finding, then continue the current task.

## Phase 0.1: Worktree Isolation

If `determinus_worktree_create` unavailable → `[ADV:BLOCKED] Worktree tools required`.

`git worktree list --porcelain` → find `change/{change-id}`:
- Exists → reuse, switch `workdir`
- Missing → `git worktree prune` then create

Create: `determinus_worktree_create branch: "change/{change-id}"`. Capture path; use it for ALL subsequent calls. To delete later, first call `determinus_worktree_delete branch: "change/{change-id}" dryRun: true`, retain the exact returned `planToken`, then apply with that token and nonblank approval evidence. A branch-only destructive call returns `PLAN_REQUIRED`.
...
Check `determinus_change_list` for active changes. Compare affected files.
...
Tasks may target other repositories. See `determinus_INSTRUCTIONS.md § Cross-Repo Execution`.
...
When task contributes to another determinus-enabled project, use ADV tools with explicit `target_path`.

| Operation | Required behavior |
| --- | --- |
| Target reads | `determinus_change_show`, task/gate/status reads with `target_path`; inspect `_projectContext` |
...
All cancellations require explicit user approval via `determinus_task_cancel`. Irreversible. Strict regex parsing per `docs/command-voice-standard.md`.
...
Anchor: `approve all`. On approval: `determinus_task_checkpoint mode: 'cancel'` then `determinus_task_cancel approvedByUser: true`.
...
```
determinus_change_show changeId: <target> include: { ledger: true, snapshot: true, readyTasks: true, briefingPacket: true }
```
...
- Planning gate pending → complete planning with `determinus_gate_complete changeId: {change-id} gateId: planning userApproved: true`; `/determinus-apply {change-id}` counts as approval
- Prep gate not complete (and not planning pending) → stop; require `/determinus-prep`

× MUST NOT ask "Begin work?" when prep has `userApproved`.

## Phase 2: Display Contract

Emit: `Working on: {change-id}`. State visible via `determinus_change_show include.snapshot:true`.
...
Each attempt must differ. After each failed attempt: `determinus_task_update` with `error_recovery` payload.
...
A valid `determinus_subagent_report_submit` retry after a single empty/malformed incident resolves the recovery state to clean; it does **not** record inline diagnosis evidence. Malformed/empty worker output stays in `delegation_recovery`, never in `error_recovery.attempts`.
...
Two tiers:
- **Phase start:** `determinus_change_show` → full context + `epic_membership` and EPIC CONTEXT from the briefing packet
- **Per task:** `determinus_task_show` → task details; `determinus_wisdom_list` → learnings

× Do NOT call `determinus_change_show` before every task.
...
TodoWrite is a projection over ADV tasks. Copy `_todoProjection` rows from `determinus_task_ready` or `determinus_change_show include.readyTasks:true`. Unknown `tk-*` IDs, other-change IDs, and premature `completed` are blocked during top-level ADV execution; subagent scratchpads remain allowed.

### Ops Runbook Execution

For `ops` tasks or contract refs requiring production ops evidence, use `determinus_ops_run_upsert` with env, action, bounds, evidence policy, rollback plan, and execute step approval. Runs are either `bounded_low_risk_autonomous` or `approval_required`. Append evidence with `determinus_ops_run_evidence_add`. Missing approval/evidence → keep task `in_progress`, surface blocker.
...
`determinus_run_test` for ordinary inline red/green. Durable proof recorded on `taskCompletedSignal.verification`.
...
| 2 | `tdd_intent == "not_applicable"` or trivial title | `delegate_allowed` to `determinus-engineer` |
...
| 4.5 | Context-shed test passes? (4-question AND, floor ~5 files or ~50 lines) | `delegate_allowed` to `determinus-engineer` |
| 5 | Default | `inline_required` |

`metadata.frontend == "true"` is structural classification. Classified UI/component implementation is engineer-first: it starts with `determinus-engineer` when delegated, or inline when risk forces inline. After successful same-cycle implementation, ADV dispatches a matching-cycle `determinus-designer` follow-up with its `IMPLEMENTATION_RECEIPT`. Priority 1 (`metadata.delegation_hint`) remains the explicit user override; it cannot select `determinus-designer` as initial implementation for a classified frontend task. Step 4 risk signals still force inline. Step 4.5 does not override Step 1 or Step 4.

`determinus-designer` remediates in scope (UI/a11y/polish fixes, then verifies) — it is **not** review-only; review/harden ownership remains with `determinus-reviewer`.
...
**Verify-burst delegation:** Heavy verification (output >200 lines, >30s, combined lint/typecheck/tests) may use `determinus-verifier`. Inline for focused `determinus_run_test`. Packet templates in `docs/apply-subagent-packets.md`.

**If `inline_required`:** standard TDD flow.

Emit routing summary: `tk-{id} → {inline|determinus-engineer|determinus-designer|determinus-verifier|general} ({reason})`

#### Verify-Burst Delegation

Task-level delegation covers implementation. Heavy verification bursts — full lint, project-wide typecheck, broad test suites — are candidates for `determinus-verifier` even during inline work. Use general unavailable-runtime fallback only if `determinus-verifier` cannot be spawned.

When to delegate: output expected >200 lines, runtime >30s, combined lint/typecheck/tests, or timeout isolation needed.
When to keep inline: focused `determinus_run_test` red/green, single-file lint, short expected output.

#### Verification Triage Packet

Spawn determinus-verifier via subagent_type: "determinus-verifier"; general unavailable-runtime fallback only.
...
BACKEND BOUNDARY: if the UI task requires changing storage, APIs, persistence/state-store changes to the change record, or business logic, stop and report. Populate scope_drift.recommendation: "stop_and_report" and required_main_agent_actions with a handoff to `determinus-engineer`. Do NOT edit backend files.
PROJECT STRUCTURE: {brief ls or glob output showing relevant directories/files in workdir}
DESIGN EXCERPT: {relevant section if task references design}
...
For each ready task from `determinus_task_ready`:

1. **Start:** `determinus_task_show` + `determinus_wisdom_list` → `determinus_task_update status: "in_progress"`
...
4. **Red:** Failing test with `edit`/`write`/`morph_edit` → `determinus_run_test phase:'red'`.
5. **Green:** Implement → `determinus_run_test phase:'green'`. If fails: retry protocol.
6. **Verify (optional):** `determinus_run_test phase:'verify'` when distinct from green.
7. **Incremental verification:** After each task run build/tests/lint for scope. Must pass before checkpoint.
8. **Checkpoint:** `determinus_task_checkpoint` with `taskId`, `workdir`, `changeId`, `expectedBranch`, `expectedHeadSha`, `verification`. If `checkpointRecorded:true` → done. If `failed` SEMANTIC → retry; ENVIRONMENTAL → escalate; TRANSIENT → surface as SEMANTIC/ENVIRONMENTAL.
...
10. **Complete:** assert `checkpointRecorded:true`; do NOT call `determinus_task_update status: "done"`.
11. **Loop:** `determinus_task_ready` → if ready tasks remain, repeat.
...
4. User-requested cancellation → `determinus_task_cancel` flow
5. Scope expansion requiring reentry → `determinus_change_reenter`
...
Task state visible via `determinus_task_list`. Use `include.snapshot:true` on mutation tools or `determinus_change_show include.snapshot:true` to request `_contextSnapshot`. TodoWrite projection is exempt.
...
`determinus_change_show changeId: <target> validate: true` → must pass.

### Mark Gate

`determinus_gate_complete changeId: {change-id} gateId: execution`
...
| State | Tool |
|---|---|
| Task status | `determinus_task_update` |
| Task list | `determinus_task_list` |
| Ready tasks | `determinus_task_ready` |
| Change data | `determinus_change_show` |
| Validation | `determinus_change_show validate: true` |
