...
`skill("determinus-clarify")` → Socratic types, ambiguity categories, funnel technique, findings-driven mode, resolution log, output summary. Required — if the skill fails to load, stop and report a broken deploy (run scripts/deploy-local.sh --fix).
